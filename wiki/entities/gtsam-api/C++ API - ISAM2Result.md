---
type: entity
tags: [GTSAM, C++ API, ISAM2, ISAM2Result]
created: 2026-05-29
updated: 2026-05-29
sources:
  - https://gtsam.org/doxygen/
  - raw/codes/gtsam
---

# GTSAM::ISAM2Result

> **结构体** | 头文件: `ISAM2Result.h` | [在线文档](https://gtsam.org/doxygen/)

## 构造函数

```cpp
ISAM2Result(bool enableDetailedResults)
```

## 公开方法

### 方法

```cpp
DetailedResults * details()
```
Return pointer to detail, 0 if no detail requested.

```cpp
print(const std::string str = "") const
```
Print results.

```cpp
size_t getVariablesRelinearized() const
```

```cpp
size_t getVariablesReeliminated() const
```

```cpp
FactorIndices getNewFactorsIndices() const
```

```cpp
size_t getCliques() const
```

```cpp
size_t getTreeNnz() const
```

```cpp
bool getBatchReorderTriggered() const
```

```cpp
double getErrorBefore() const
```

```cpp
double getErrorAfter() const
```

## 公开成员变量

```cpp
std::optional< double > errorBefore
```
```cpp
std::optional< double > errorAfter
```
```cpp
size_t variablesRelinearized
```
```cpp
size_t variablesReeliminated
```
```cpp
size_t factorsRecalculated
```
```cpp
size_t cliques
```
```cpp
size_t treeNnz
```
```cpp
bool batchReorderTriggered
```
```cpp
FactorIndices newFactorsIndices
```
```cpp
KeySet unusedKeys
```
```cpp
KeyVector observedKeys
```
```cpp
KeySet keysWithRemovedFactors
```
```cpp
KeySet markedKeys
```
```cpp
std::optional< DetailedResults > detail
```

## 详细说明

This struct is returned from ISAM2::update() and contains information about the update that is useful for determining whether the solution is converging, and about how much work was required for the update. See member variables for details and information about each entry.

## 源码位置

- 分支: `develop`
- 远程: https://github.com/borglab/gtsam.git
- 本地快照: `raw/codes/gtsam`

## Agent 实现提示

### 适用场景

`ISAM2Result` 用于 GTSAM factor graph 优化流程中。

### 输入输出契约

参见上方 [公开方法](#公开方法) 的签名。

### 实现注意事项

- 所有 Lie group 类型使用右扰动（right perturbation）约定
- 使用 `OptionalJacobian` 参数可选的链式求导

## 相关页面

- [[方法-GTSAM-API族]]
- [[GTSAM Geometry API]]
- [[GTSAM API 使用索引]]
- [[GTSAM 4.3a1 使用指南]]
