...
| 传感器 | 频率 | 原始格式 | 接口 | 消费模块 |
|--------|------|----------|------|----------|
| LiDAR (Velodyne/Ouster/等) | 10Hz | `sensor_msgs::PointCloud2` (ROS1/ROS2) / 数据集loader | `pin_slam_ros.py:PointCloud2` 回调 / `data_sampler.py` | tracker → mapper |
| IMU | - | 不使用 | PIN-SLAM是纯LiDAR系统 | - |

### 7.2 LiDAR→Neural Points管线

```
原始PointCloud2 (ROS1/ROS2) 或 数据集文件(.bin/.pcd)
  → data_sampler.py / point_cloud2.py: 反序列化 → torch.Tensor (N,3) 世界坐标系
  → pin_slam.py::run():
      tracker.tracking(source_points, initial_pose):
        step 1: 体素降采样 (source_vox_down_m=0.8m)
        step 2: 迭代 (max 50次):
          a. transform to world: p_world = T * p_source
          b. query_source_points(): 空间中每个p_world →
               neural_points.query_feature(p):
                 spatial hash → KNN (K个neural points)
                 IDW反距离加权融合K个点的geo_features → 1个特征向量
               decoder.sdf(feature): MLP前向 → SDF值 + 分析梯度(sdf_grad)
          c. SDF残差: residual = sdf_pred (理想为0, 点在表面上)
          d. Jacobian: J = [cross(p_world, sdf_grad), sdf_grad] (N×6)
          e. LM求解: (J^T*W*J + λ*diag)*Δx = -J^T*W*residual
          f. Geman-McClure鲁棒权重:
               w_grad = (GM_grad/(GM_grad+grad_anomaly²))² ×
               w_res = (GM_dist/(GM_dist+sdf_residual²))² ×
               w_normal = 0.5+|normal·grad_unit| ×
               w_color = exp(-|color_pred-color_meas|)
          g. 收敛检查: ΔR<0.01° && ΔT<0.001m
        step 3: Hessian特征值检查 → 退化检测 → valid_flag
        step 4: 失败时连续丢帧≥5 → reboot_ts重启动(清空data pool + 解冻decoder)
  → mapper.process_frame(frame_pose, source_points):
      采样点: 在source_points周围 + random free space采样
      → query SDF(采样点) → 计算BCE/MAE loss
      → optimizer.step(): 梯度回传到 neural_points特征 + decoder权重
      → neural_points更新: 新点通过spatial hash合并/插入
      → 置信度累积: local_point_certainties.scatter_add_(weight_vector)
```

**标定**: 无外参(纯LiDAR世界系=首帧LiDAR系), 无内参  
**预处理**: 体素降采样(0.8m) → NaN/范围滤波(数据集loader中)  
**特征**: SDF隐式特征: 通过neural points(3D位置+8维特征向量+方向四元数+置信度)编码局部几何  
**匹配**: Spatial hash O(1)最近邻搜索 → KNN neural points → IDW特征融合 → MLP解码SDF  
**因子构建**: 点-SDF隐式LM: `J(1×6) = [cross(p, grad)^T, grad^T]`, 自动微分获得梯度, 不需显式point-to-plane雅可比

### 7.3 Neural Point Map Context管线 (回环检测)

```
local neural points (当前位姿周围)
  → ptcloud2sc_torch(): 投影到极坐标 (20 ring × 60 sector)
  → 每扇区存最高点高度值 → Scan Context矩阵
  → sc2rk(): 每环取平均 → ring key (20维旋转不变向量)
  → 两阶段匹配:
      阶段1: ring key KD-tree粗筛 → top-k候选
      阶段2: Scan Context列对齐精匹配 → 最佳旋转角
  → set_virtual_node(): 虚拟节点增广(横向偏移) → 解决平移不匹配
  → 回环候选验证: 完整scan-to-implicit-map配准 → reg_valid_flag=true 才加入PGO
```

**特征**: Neural Point Map Context (基于neural points而非原始点云的Scan Context)  
**匹配**: Ring key快速检索 + 列对齐精匹配 + 虚拟节点增广

### 7.4 跨传感器/模块协同

| 协同机制 | 实现位置 | 说明 |
|----------|----------|------|
| 时间同步 | 以LiDAR帧时间戳为准 | 纯LiDAR系统无多传感器时间对齐 |
| 缓冲策略 | data pool (mapper.process_frame累积) | 滑动窗口存储最近frames的source points用于mapping训练 |
| 初始化 | 首帧480次迭代(40×12) | `init_iter_ratio × iters`, 构建初始neural points+训练decoder |
| tracker→mapper | frame_id, pose, source_points传递 | 异步: tracking输出位姿, mapping独立训练 |
| mapping→tracker | neural points + decoder共享 | tracker用最新neural points做配准, 不等待mapping完成 |
| 回环→地图校正 | adjust_map(pose_diff) | PGO后所有neural points按时间戳关联的位姿修正量刚性变换 |
| 降级策略 | consecutive_lose_track≥5 → reboot | 清空data pool, 解冻decoder重新学习 |
| 可微地图统一框架 | neural points特征 + decoder权重 | tracking/mapping/loop closure共享同一neural points表示 |