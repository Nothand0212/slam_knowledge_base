...
| 传感器 | 频率 | 数据格式 | 接口 | 负责模块 |
|--------|------|----------|------|----------|
| FAST-LIO2 Odometry | ~10 Hz (与 LiDAR 同步) | `nav_msgs::Odometry` (/Odometry) | `fast_lio_sam_sc_qn.cpp:76-78` | PosePcd 构造函数 |
| FAST-LIO2 PointCloud | ~10 Hz | `sensor_msgs::PointCloud2` (/cloud_registered) | `fast_lio_sam_sc_qn.cpp:76-78` | PosePcd 构造函数 |

**注意**：本仓库自身**不直接订阅 LiDAR/IMU 硬件 topic**。所有传感器数据由外部 FAST-LIO2 节点预处理后以 odometry + 世界坐标系点云的形式输入。

### 9.2 LiDAR-IMU（FAST-LIO2 前端）管线

#### 9.2.1 原始数据（FAST-LIO2 内部）

外部 FAST-LIO2 节点处理：
- LiDAR 原始点云 → IESKF + 反向传播去畸变 → 世界坐标系已配准点云
- IMU 高频数据 (≥200 Hz) → 紧耦合 IESKF 状态估计
- 输出 `/Odometry`（18 维状态估计转换的 6DoF 位姿）+ `/cloud_registered`（已去畸变全局对齐点云，`PointXYZI` 格式）

FAST-LIO2 状态向量：`[pos(3), rot(3), vel(3), bg(3), ba(3), gravity(3)]`（18 维）。
- 使用 **ikd-Tree** 维护增量地图
- **点到平面 + 点到边缘** 复合残差

#### 9.2.2 预处理（本仓库层）

**时间同步**（`fast_lio_sam_sc_qn.h:50`）：
```
typedef message_filters::sync_policies::ApproximateTime<Odometry, PointCloud2> odom_pcd_sync_pol;
```
使用 `message_filters::ApproximateTime` 同步策略，Odom 和 PCD 话题对齐后送入 `odomPcdCallback()`（`fast_lio_sam_sc_qn.cpp:87-206`）。

**数据解析**（`include/pose_pcd.hpp:21-43`）：
1. 从 odom 消息提取 `pose_eig_`（4x4 Eigen 矩阵，世界系位姿）
2. 从 pcd 消息解序列化为 `pcl::PointCloud<PointType>`（`PointType = pcl::PointXYZI`）
3. **逆变换存储**（`pose_pcd.hpp:40`）：
   ```cpp
   pcd_ = transformPcd(tmp_pcd, pose_eig_.inverse());
   // FAST-LIO outputs in world frame → store in LiDAR frame
   ```
   FAST-LIO2 输出世界坐标系点云，此代码将其**转回 LiDAR 坐标系存储**，便于后续回环重新投影。

#### 9.2.3 特征提取

本仓库**不进行特征提取**。FAST-LIO2 前端使用原始点的直接点到平面/点到边匹配，无需预先提取 edge/planar 特征。本仓库仅做 PGO + 回环检测。

#### 9.2.4 关键帧选取

**策略：欧式距离阈值**（`fast_lio_sam_sc_qn.cpp:503-506`）：
```cpp
keyframe_thr_ < (latest_pose_corrected.tr - pose_pcd_in.tr).norm()
```
配置默认 `keyframe_threshold: 1.5` 米（`config.yaml:7`）。仅平移距离超过阈值即创建新关键帧。

**关键帧生命周期**（`fast_lio_sam_sc_qn.cpp:126-150`）：
1. 存入 `keyframes_` 向量
2. 更新 ScanContext 全局描述子数据库
3. 添加 PGO 图因子（与前一关键帧的 odom between factor）
4. ISAM2 增量优化
...
**阶段 1：ScanContext 全局描述子检索**（`loop_closure.cpp:34-56`）：
- 每个关键帧调用 `sc_manager_.makeAndSaveScancontextAndKeys(cloud)` 生成描述子
- 回环候选：`sc_manager_.detectLoopClosureIDGivenScan(query)` （column-wise 距离 + 行平移对齐）
- **二次过滤**：欧式距离 < 35m（`scancontext_max_correspondence_distance`）

**阶段 2：Quatro 粗对齐**（`loop_closure.cpp:138-144`）：
- 基于 **FPFH + GNC (Graduated Non-Convexity)** 的全局点云配准
- 参数：`fpfh_normal_radius=0.9`, `fpfh_radius=1.5`, `noise_bound=0.3`
- 对初始猜测不敏感，可在无先验信息下完成全局匹配

**阶段 3：Nano-GICP 精细对齐**（`loop_closure.cpp:110-156`）：
- **NanoFLANN** 实现快速 KD-Tree 最近邻搜索
- **FastGICP** 实现体素级 GICP（分布到分布匹配）
- 计算源/目标点云的局部分布协方差：
  ```cpp
  nano_gicp_.calculateSourceCovariances();
  nano_gicp_.calculateTargetCovariances();
...
**阶段 4：Voxel Grid 降采样**（`utilities.hpp:38-63`）：
- 分辨率：0.3m（`quatro_nano_gicp_voxel_resolution`）
- 在匹配前对 src/dst 点云做 `pcl::VoxelGrid` 降采样

#### 9.2.6 算法消费（GTSAM ISAM2 PGO）

**因子图结构**：

**① 先验因子**（`fast_lio_sam_sc_qn.cpp:113-118`）：
```
PriorFactor<Pose3>(0, pose, noise=[1e-4 (rot), 1e-2 (trans)])
```

**② 里程计 BetweenFactor**（`fast_lio_sam_sc_qn.cpp:135-146`）：
```
BetweenFactor<Pose3>(idx-1, idx, poseFrom.between(poseTo), odom_noise)
```
噪声同上：`[1e-4, 1e-4, 1e-4, 1e-2, 1e-2, 1e-2]`

**③ 回环 BetweenFactor**（`fast_lio_sam_sc_qn.cpp:228-238`）：
```
BetweenFactor<Pose3>(query, candidate, poseBetween, loop_noise)
```
噪声 = `ICP_score.repeated<6>()`。**score 越小（匹配越好）→ 信息矩阵权重越大**。

**ISAM2 配置**（`fast_lio_sam_sc_qn.cpp:54-57`）：
```
relinearizeThreshold = 0.01, relinearizeSkip = 1
```
回环后额外 3 次 `update()` 确保收敛。

**实时位姿外推**（`fast_lio_sam_sc_qn.cpp:94-104`）：
```cpp
odom_delta = last_odom_tf.inverse() * current_frame.pose_eig_;
pose_corrected = last_corrected_pose * odom_delta;
```
用最近一次 ISAM2 结果作为基准，FAST-LIO2 的增量 odometry 做高频实时位姿外推。回环发生后 `odom_delta_` 重置为 Identity。

### 9.3 地图管线

**点云存储**：关键帧点云存储为 LiDAR frame 下的坐标（`pose_pcd.hpp:39-41`）

**Voxel Grid 降采样**（`fast_lio_sam_sc_qn.cpp:308-322`）：全局地图拼接 + 体素降采样（resolution = 0.3m）

**输出格式**：支持 .bag, .pcd, KITTI 三种格式（`config.yaml:43-47`）
...
FAST-LIO2 节点 (外部进程)
  ...
  └─ /cloud_registered ──┘
        │
        ▼
  ApproximateTime Sync (message_filters)
        │
        ▼
  odomPcdCallback → PosePcd 构造 → 关键帧判断 → ISAM2 PGO
```

**降级**：
- 若 FAST-LIO2 前端漂移过大，ScanContext 回环检测可能失效
- 回环因子的信息矩阵直接引用 ICP score，未区分位置/旋转的信息量
- `ApproximateTime` 同步可能引入 ms 级时间偏差
