...
|--------|------|----------|---------|----------|
| Livox LiDAR | 10Hz | `livox_ros_driver::CustomMsg` (含逐点time/ring) | `feat_points_cbk` → `lidar_buffer` | LIO线程 |
| IMU (内置/外置) | 200Hz | `sensor_msgs::Imu` | `imu_cbk` → `imu_buffer_lio` + `imu_buffer_vio` | LIO+VIO线程(独立双缓冲) |
| 全局快门相机 | 10-30Hz | `sensor_msgs::Image` / `CompressedImage` | `image_cbk` → `img_buffer` + 异步处理线程 | VIO线程 |

### 8.2 LiDAR管线

```
原始Livox CustomMsg
  → 外部节点(特征提取): 下采样 → feats_down (降采样后特征点)
  → r3live_lio.cpp:feat_points_cbk(t+0.1): 入队 lidar_buffer
  → sync_packages(): 取最老帧 + 对应IMU → MeasureGroup
  → p_imu->Process(): 前向传播去畸变(前后向补偿)
  → 去畸变点云 → RGB着色: RGBpointBodyToWorld() → append_points_to_global_map()
```

**标定**: LiDAR-IMU外参预标定(const), 内参无特殊处理  
**预处理**: 在外部节点完成特征提取(边缘/平面)，R3LIVE消费 `feats_down`  
**特征**: 非R3LIVE内部提取，消费的是下采样后的特征点  
**匹配**: iKd-Tree Nearest_Search → 5点PCA平面拟合 → 有符号距离 `pa*x+pb*y+pc*z+pd`  
**因子构建**: 点到平面雅可比 `Hsub = [point_crossmat * R^T * n, n]` (1×6)

### 8.3 视觉管线

```
原始图像(BGR8/Compressed)
  → r3live_vio.cpp:image_cbk(): 首帧启动service_process_img_buffer线程
  → process_image(): resize(scale_factor) → cv::remap(去畸变) → init_cubic_interpolation → image_equalize
  → 去畸变均衡化后图像 → m_queue_image_with_pose (带LIO先验位姿)
  → Rgbmap_tracker::track_img(): LK光流跟踪RGB地图点到当前帧
  → 跟踪结果: m_map_rgb_pts_in_current_frame_pos (RGB_pts* → cv::Point2f)
```

**标定**: 相机内参(fx,fy,cx,cy) + IMU-相机外参(R_ext, t_ext, td) 在线估计  
**预处理**: resize → remap(initUndistortRectifyMap预计算) → 直方图均衡  
**特征**: 不是传统角点而是LiDAR着色后的全局RGB地图点(已着色的3D点)  
**匹配**: LK金字塔光流(lkpyramid.cpp)帧间跟踪 + 全局地图点重投影  
**因子构建**: 重投影误差雅可比 → `∂proj/∂δθ = mat_pre*Rci*skew(p_body_imu)`, `∂proj/∂δp = -mat_pre*Rci*R_imu^T`, 含外参/内参/时间偏移链式求导

### 8.4 IMU管线

```
sensor_msgs::Imu (200Hz)
  → imu_cbk(): 时间合法性检查 → 同时推入imu_buffer_lio + imu_buffer_vio (双缓冲零拷贝)
  → LIO侧: ImuProcess::Process() → 前向传播(R_k+1 = R_k*Exp(angvel-b_g, dt)) → 协方差传播
  → VIO侧: 独立ImuProcess消费imu_buffer_vio → vio_preintegration() 对齐图像时间戳
```

**预处理**: 加速度乘G配置(`m_if_acc_mul_G`), 不做其他滤波  
**因子**: LIO中IMU作为状态传播(预测步)，VIO中IMU预积分构建先验约束 `(x_propagate - x_current)`

### 8.5 跨传感器协同

| 协同机制 | 实现位置 | 说明 |
|----------|----------|------|
| 时间同步 | `g_camera_lidar_queue` | 共享队列管理LiDAR/相机/IMU时间戳对齐，`lidar_in(t+0.1)` 补偿时间偏移 |
| 缓冲策略 | `imu_buffer_lio` + `imu_buffer_vio` | IMU同时入两个独立deque，两线程各自消费无竞争 |
| 初始化顺序 | LIO先启动 → VIO等待≥100个RGB地图点 | `while(m_map_rgb_pts.size() <= 100)` spin等待 |
| LIO→VIO先验 | `vio_preintegration()` | LIO位姿作为VIO起点，IMU预积分到图像时间戳 |
| 降级策略 | VIO失败时保持LIO输出 | VIO依赖LIO先验，视觉track丢失不影响LiDAR里程计 |
| 互斥锁 | `m_mutex_lio_process` | 保护g_lio_state读写，LIO写/VIO读
