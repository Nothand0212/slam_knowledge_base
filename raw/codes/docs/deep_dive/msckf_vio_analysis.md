...
|--------|------|----------|-------------|----------|
| 相机 (stereo) | 10-30 Hz | `sensor_msgs::Image` → `cv::Mat` | `cam0_image`, `cam1_image` → `image_processor::stereoCallback` | `ImageProcessor` (via `message_filters::TimeSynchronizer`) |
| IMU | 100-200 Hz | `sensor_msgs::Imu` (acc[3], gyr[3]) | `imu` → image_processor + vio `imuCallback` | `ImageProcessor`, `MsckfVio` |

### 8.2 相机数据处理管线

#### 原始数据
- **图像格式**: `cv_bridge::toCvCopy` → `cv::Mat` (Gray/mono8)
- **双目同步**: `message_filters::TimeSynchronizer<Image,Image>` 硬件同步，同时触发 `stereoCallback`
- **标定来源**: Kalibr 格式 YAML (`camchain-imucam-*.yaml`), 通过 ROS param server 加载
- **内参**: `[fx, fy, cx, cy]`, 畸变系数 `[k1,k2,p1,p2]`
- **畸变模型**: `radtan` (`cv::undistortPoints`) / `equidistant` (`cv::fisheye::undistortPoints`)
- **外参**: `T_imu_cam0`, `T_imu_cam1` 链式计算 — 从 `cam0/T_cam_imu`, `cam1/T_cn_cnm1` (行 90-101)

#### 前端跟踪 (`ImageProcessor::trackFeatures`, `src/image_processor.cpp:417-607`)

**Step 0: IMU 运动预测** (`predictFeatureTracking`, 行 387-415)
- `integrateImuData` 取帧间陀螺仪均值 → 罗德里格斯公式 → 帧间旋转 `R_p_c`
- 补偿: `compensated_pt = K·R_p_c·K^{-1}·prev_pt` (纯旋转单应)

**Step 1: KLT 光流时序跟踪** (行 459-468)
- `cv::calcOpticalFlowPyrLK`: `pyramid_levels=3`, `patch_size=15×15`, `max_iteration=30`, `track_precision=0.01`, `OPTFLOW_USE_INITIAL_FLOW`
- 金字塔: `cv::buildOpticalFlowPyramid` 构建

**Step 2: 双目立体匹配** (`stereoMatch`, 行 609-691)
- cam0 去畸变 → `R_cam0_cam1` 旋转 → 加畸变 → cam1 预测位置
- `cv::calcOpticalFlowPyrLK` cam0→cam1 精化
- **极线校验** (行 653): `E = [t]×·R_cam0_cam1` → 每对匹配点极线距离 < `stereo_threshold·norm_pixel_unit`

**Step 3: 自写 2 点 RANSAC** (`twoPointRansac`, 行 987-1230)
- 已知 IMU 旋转 → 位移仅由平移引起 → 模型: `coeff_t·[tx,ty,tz]^T ≈ 0`
- 2 点随机采样 → 解 2×2 组 → 内点>20% → LS 重拟合
- 退化检测: 平均位移 < 1px → 纯旋转，仅距离阈值筛
- cam0 和 cam1 的时序匹配分别执行，两者都通过才保留

**Step 4: 新特征补充** (`addNewFeatures`, 行 693-832)
- 已有特征 5×5 邻域 mask → `cv::FastFeatureDetector::create(fast_threshold)` (EuRoC=10)
- 网格: `grid_row×grid_col` (EuRoC: 4×5=20), 每格 `grid_max_feature_num` 个 (EuRoC=4)
- 仅对不足 `grid_min_feature_num` (EuRoC=3) 的网格补充

**Step 5: 剪枝与发布** (`pruneGridFeatures` + `publish`)
- 超限网格按生命周期排序保留最久特征
- 发布前去畸变: `cv::undistortPoints` / `cv::fisheye::undistortPoints` → 归一化坐标

### 8.3 IMU 数据处理管线

#### 原始数据与缓冲
- **测量**: `sensor_msgs::Imu` → `acc[3] m/s², gyr[3] rad/s`
- **image_processor**: `imu_msg_buffer` (vector)，取帧间陀螺仪均值做旋转预测；用后 `erase` 清理
- **vio**: `imuCallback` → `imu_msg_buffer`；`batchImuProcessing` (行 508-538) 按需批量处理 `[last_state.time, image.time]` 间数据

#### IMU 传播 (`MsckfVio::processModel` + `predictNewState`, `src/msckf_vio.cpp:508-692`)

**状态预测 (RK4)**:
```
dq/dt = (cos(|w|·dt/2)·I + sin(|w|·dt/2)/|w|·Ω)·q   // 四元数闭式积分
v += dt/6·(k1_v + 2k2_v + 2k3_v + k4_v)              // RK4 速度
p += dt/6·(k1_p + 2k2_p + 2k3_p + k4_p)              // RK4 位置
```

**误差状态转移** (3 阶矩阵指数近似, 行 571-575):
```
Fdt = F·dt, Fdt_sq = Fdt·Fdt, Fdt_cube = Fdt_sq·Fdt
Φ = I + Fdt + 0.5·Fdt_sq + (1/6)·Fdt_cube
```

- F(21×21): `F(0:3,0:3)=-[gyro]×`, `F(0:3,3:6)=-I`, `F(6:9,0:3)=-R^T·[acc]×`, `F(6:9,9:12)=-R^T`, `F(12:15,6:9)=I`

**OC-MSCKF 修正** (`msckf_vio.cpp:580-597`): 修正 Phi 保持不可观子空间一致性
```
Phi(0:3,0:3) = R_k·R_{k-1}^T
Phi(6:9,0:3) = A1 - (A1·u-w1)·s    // 速度 yaw 不可观方向修正
Phi(12:15,0:3) = A2 - (A2·u-w2)·s  // 位置 yaw 不可观方向修正
```

**协方差传播**: `Q = Φ·G·Qc·G^T·Φ^T·dt`, `P = Φ·P·Φ^T + Q`

#### 静态初始化 (`initializeGravityAndBias`, `msckf_vio.cpp:248-284`)
1. 收集 ≥200 条 IMU
2. `gyro_bias = sum(w)/N` (均值)
3. `gravity_imu = sum(a)/N` → 世界系 `[0,0,-|gravity_imu|]`
4. `orientation = FromTwoVectors(gravity_imu, -gravity_world)`
5. acc_bias=0 (不可观), pos=origin, vel=0/config

### 8.4 算法消费：MSCKF 因子构造

#### 测量雅可比 (`measurementJacobian`, `msckf_vio.cpp:789-860`)
```
双目观测 4 维: z = [u0, v0, u1, v1]
z_hat = [Pc.x/Pc.z, Pc.y/Pc.z, Pc_right.x/Pc_right.z, Pc_right.y/Pc_right.z]
H_x = ∂z_hat/∂xcam (4×6, 测量对相机状态)
H_f = ∂z_hat/∂Pf (4×3, 测量对特征 3D)
```

**OC 修正** (行 844-853): 消除不可观方向
```
A = H_x, u = [R_null·g; [p_w-p_null]×·g]
H_x = A - A·u·(u^T·u)^{-1}·u^T
H_f = -H_x(:,0:3)
```

#### 特征 3D 初始化 (`Feature::initializePosition`, `feature.hpp:293-437`)
- 逆深度参数化 `(α,β,ρ) = (x/z, y/z, 1/z)`
- 自写 **LM 优化**: 外循环 10 次，内循环 10 次，阻尼 `λ` 自适应 (cost↓→λ/10, cost↑→λ*10)
- Huber 核 ε=0.01, 收敛阈值 `estimation_precision=5e-7`

#### 零空间投影 (`featureJacobian`, `msckf_vio.cpp:862-916`)
```
JacobiSVD(H_fj) → U 的右列 (H_fj 的左零空间) = A
H_x = A^T·H_xj, r = A^T·r_j          // 消去特征 3D 坐标
```

#### 卡方门控 + EKF 更新 (`measurementUpdate`, `msckf_vio.cpp:918-1020`)
```
γ = r^T·(H·P·H^T + σ²·I)^{-1}·r < χ²_{0.95}(dof)   // Boost chi_squared_dist
QR 压缩: H → SPQR → 薄 H (H 行 > 列)
K = P·H^T·(H·P·H^T + σ²·I)^{-1}                        // LDLT 求解
x = x + K·r, P = (I-K·H)·P
```

#### 相机状态增广 (`stateAugmentation`, `msckf_vio.cpp:695-755`)
```
R_w_c = R_i_c·R_w_i, t_c_w = p_w_i + R_w_i^T·t_c_i
P_new = [P, P·J^T; J·P, J·P·J^T]      // 6×21 雅可比 J
```

### 8.5 跨传感器协同

#### 时间同步
- `message_filters::TimeSynchronizer` 双目硬件同步
- IMU 缓冲 `imu_msg_buffer` 处理传输延迟
- `batchImuProcessing`: 所有 ≤ image.time 的缓冲 IMU 批量处理

#### 滑动窗口管理 (`pruneCamStateBuffer`, `msckf_vio.cpp:1180-1305`)
- `max_cam_state_size=20` (EuRoC)
- `findRedundantCamStates`: 以倒数第 4 帧为参考，检查旋转/平移/跟踪率
  - `angle<0.2618rad(15°)` & `distance<0.4m` & `tracking_rate>0.5` → 删冗余
  - 否则删最老 2 帧
- 删除前对关联特征做 MCCKF 更新，充分利用数据

#### 降级处理
- `onlineReset`: 位置标准差 > 8.0m 时清空相机状态+特征，保留 IMU 状态
- 无回环检测、无全局 BA、无位姿图优化