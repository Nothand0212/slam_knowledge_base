...
| 传感器 | 频率 | 数据格式 | 接口 | 负责模块 |
|--------|------|----------|------|----------|
| 灰度相机 (N×cam) | 20-60 Hz | `cv::Mat` (8bit gray) → `ImgMeasurement` | `ROS img_callback` → `rovio::RovioFilter::addImage()` | `ImgUpdate` |
| IMU (加速度计+陀螺仪) | 100-400 Hz | `ImuMeasurement` (acc(3), gyr(3), ts) | `ROS imu_callback` → `rovio::RovioFilter::addImu()` | `ImuPrediction` |
| 相机标定 | 一次性 | yaml → `Camera` (pinhole or equidistant) | `ROSCamera::readFromROS()` | `MultiCamera` |

### 6.2 相机管线

#### 原始数据
```
规格: 单/双目, 752×480 (Euroc) or 640×480
格式: ros::sensor_msgs::ImageConstPtr → cv_bridge → cv::Mat → ImgMeasurement
标定: Camera 类 (fx, fy, cx, cy, k1, k2, p1, p2 + 畸变模型)
```

#### 预处理
**Step 1 — 图像金字塔** (`ImagePyramid.hpp`):
```
构建多层图像金字塔 (nLevels 层, 默认 4 层)
每层下采样 2×, 存储完整像素值数组
用于多层块对齐中的粗到细搜索
```

**Step 2 — 特征预处理** (`ImgUpdate.hpp:preProcess`, ~line 380):
```
对每个活跃特征:
  1. 从 EKF 预测 bearng vector → 根据 IMU 运动学 warp
     warp 预测: ImuPrediction.hpp:124-130
     bearingVectorJac = M_transpose · (dt·camVel + I · ...)
  2. 投影到像素坐标: bearingToPixel(...)
  3. 2D 对齐: align2D() 搜索匹配位置
     使用 MultilevelPatchAlignment 做光度对齐
     输出: feaCoorMeas_[ID] (对齐后的像素坐标)
```

#### 特征提取 (Patch 管理)
**不使用传统角点检测器**。Patch 是直接提取的图像块:

**Patch 提取** (`Patch.hpp:240-304`):
```
模式 1 (无 warp, isNearIdentityWarping()):
  简单双线性插值从图像采样 patchSize×patchSize 区域
模式 2 (有 warp):
  先应用 affine warp 矩阵变换坐标, 再双线性插值
warp 信息: FeatureCoordinates::warp_c_ (2×2 像素仿射), warp_nor_ (2×2 bearing 仿射)
```

**Patch 梯度计算** (`Patch.hpp:72-101`):
```
中心差分: dx[i] = 0.5*(patch[idx+1] - patch[idx-1])
         dy[i] = 0.5*(patch[idx+refStep] - patch[idx-refStep])
Hessian (3×3): H = Σ(J·J^T), J=[dx沿dir, dy沿dir, 1]
  其中最后一项用于仿射光照补偿
Shi-Tomasi 分值: s = λ1 + λ2 (H 的前两个特征值之和)
```

**多层 Patch** (`MultilevelPatch.hpp`):
```
nLevels_ 层金字塔 patch, 每层:
  - 图像分辨率不同, patchSize 相同
  - 加权 Shi-Tomasi: s_total = Σ 0.25^i · s_i
```

#### 匹配与异常值剔除
**多层块对齐** (`MultilevelPatchAlignment.hpp`):
```
输入: 图像金字塔 + 参考 MultilevelPatch + 投影坐标 (c)
输出: 降阶雅可比 A_red (2×2) + 残差 b_red (2×1)
过程:
  从最粗层到最细层:
    对每层做光度块对齐 (inverse compositional)
    输出更新的像素坐标 + A_red/b_red 累积
```

**IEKF 候选生成** (`ImgUpdate.hpp:442-475`):
```
在像素空间中沿协方差特征向量方向采样候选点
候选数量: nCandidates (默认 ~5)
范围: 2σ 椭圆内
每个候选: 做一次 IEKF 迭代更新
```

**异常值检测** (`ImgUpdate.hpp:477-537, extraOutlierCheck`):
```
1. 迭代收敛检查: candidate 的 bearing vector 变化 < threshold
2. 图像边界检查: projected pixel 在图像内
3. 光度误差检查: 平均残差 < patchRejectionTh_ (默认 ~30)
4. 辨别性检查 (discriminative):
   在候选周围 4 个方向采样 → 至少 2/4 方向残差超过阈值
   确保 patch 确实有纹理变化, 不是偶然对齐
```

#### 算法消费 — 因子构建
**直接法 (useDirectMethod_=true)**:
```
创新项 (innovation):
  y_pix = b_red + noise  (2×1)
  b_red 是光度对齐的残差, 单位在像素级

状态雅可比:
  F = -A_red · c_J · featureOutputJac
  其中:
    A_red: 光度误差对像素坐标的降阶雅可比 (2×2)
    c_J:  像素坐标对 bearing vector 的雅可比 (2×2)
    featureOutputJac: bearing vector 对 EKF 全状态的雅可比 (2×D)

测量噪声: R = noiseGain × A_red⁻¹ (从对齐 Hessian 推导)
```

**间接法 (useDirectMethod_=false)**:
```
创新项:
  y_pix = feaCoorMeas - featureOutput.c  (2×1 重投影误差)
状态雅可比:
  F = -c_J · featureOutputJac
测量噪声: R = σ²·I (固定像素噪声)
```

**EKF 更新方程**:
```
K = P·F^T·(F·P·F^T + R)⁻¹  (Kalman Gain)
Δx = K·y
P = (I - K·F)·P
```
...
#### 预处理 — 预积分
**IMU 测量缓冲** (`ImuPrediction`):
```
ROS imu_callback → RovioFilter::addImu() → 触发 EKF 传播
每次 IMU 数据到达立即处理, 不等待图像
```

**传播方程** (`ImuPrediction.hpp:100-156`):
```
IMU 状态:
  qWM(t+dt) = qWM(t) ⊗ exp((gyr - gyb)·dt)
  MvM(t+dt) = (I - [ω]×·dt)·MvM(t) - dt·(acc - acb + qWM⁻¹·g)
  WrWM(t+dt) = WrWM(t) - dt·(qWM·MvM(t))

路标状态 (robocentric):
  bearing vector 更新: dm = -dt·(M(v)·nor + (I-nor·nor^T)·ω)
  depth 更新: d(t+dt) = d(t) - dt·(nor^T·v_cam)
```

**雅可比传播** (`ImuPrediction.hpp:161-241`):
```
F = ∂state(t+dt)/∂state(t)  (全状态雅可比)
包含:
  - IMU-IMU 交叉耦合 (位置-速度, 速度-姿态)
  - 路标-IMU 交叉耦合 (bearing vector 对 vel, gyb, att)
  - 外参-路标交叉耦合 (如果 doVECalibration_=true)
```
...
#### 时间同步
```
IMU 与相机时间戳天然同步 (ros::Time)
IMU 数据以高频到达 → RovioFilter 自动传播 EKF
图像到达时: EKF 状态已被 IMU 传播到最新时刻
  → 直接进行视觉更新 (预测-更新循环)
```

#### 数据缓冲
```
IMU: 无缓冲 (流式处理, 每条数据立即执行 EKF 传播)
图像: 无缓冲 (流式处理, 每帧到达触发 ImgUpdate)
  preProcess + IEKF 更新在当前图像回调中完成
```

#### 初始化管线
```
1. 首帧图像 + IMU:
   IMU 传播 → 检测 inertal motion (>0.1 rad/s or m/s²)
   → 首次运动时触发特征初始化
2. 特征初始化:
   FeatureManager::addFeaturesByShiTomasi() 检测角点
   → 提取 Patch → 创建 FeatureCoordinates + FeatureDistance
   → 加入 EKF 状态
3. 后续帧:
   IMU 传播 → preProcess (2D 对齐) → 直接法/间接法 EKF 更新
```

#### 降级策略
```
运动不足 (静止):
  ZeroVelocityUpdate (ZUPT): 使用零速检测约束漂移
光度误差过大:
  patchRejectionTh_ 过滤 → 特征被标记为不可靠
  满足条件时更新 patch 参考 (block update)
特征不足:
  FeatureManager::checkFeatureObservability() 检测
  → 删除不可观特征 → 添加新特征
```
