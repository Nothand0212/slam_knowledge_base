...
| 传感器 | 数据类型 | 结构体 | 坐标帧 | 频率约定 |
|--------|---------|--------|--------|---------|
| **IMU** | `ImuMeasurement` | accel(3), gyro(3), stamp | body frame | 未约束 (接口层) |
| **GNSS** | `GnssMeasurement` | positionEcef(3), positionValid, stamp | ECEF (硬编码) | 未约束 |
| **视觉** | `VisualObservation` | cameraId, stamp, featureCount | -- (无姿态) | 未约束 |
| **多相机组** | `VisualFrame` = `vector<VisualObservation>` | 各相机同步帧 | bodyFromCamera | cameraId 索引 |

### 9.2 IMU 数据管线 (骨架 + 设计意图)

```
IMU 原始硬件 (-> 上层 ROS2 wrapper 转换后)
  ↓ (ImuMeasurement: types.hpp:8-13)
accel (m/s^2, body), gyro (rad/s, body), stamp (Seconds)
  ↓ Pipeline::pushImu() (pipeline.cpp:8-11):
    m_imuQueue.push_back(m); trimQueues() -> max 4096 entries
  ↓ Pipeline::runStep() (pipeline.cpp:38-43):
    backend->onStep(tick++, imu_queue.size(), visual_queue.size(), gnss_queue.size())
    清空所有队列
  ↓ IOptimizationBackend::onStep() (接口, 优化后端):
    tick: uint64_t (单调递增); 队列长度 -> 后端需额外机制访问实际数据

[计划实现路径] (phad_fusion):
  ↓ IImuIntegrator::integrate() (sensor_interfaces.hpp:7-9):
    纯虚接口 -> 上层 GTSAM PreintegratedImuMeasurements
  ↓ 预积分残差 -> CombinedImuFactor -> GTSAM graph
  ↓ 偏置: 当前 ImuMeasurement 不含 ba/bg -> 需扩展
```

### 9.3 GNSS 数据管线 (骨架 + 扩展需求)

```
GNSS 接收机 (-> ROS2 NavSatFix -> wrapper 转换)
  ↓ (GnssMeasurement: types.hpp:15-19)
positionEcef (ECEF, m), positionValid (bool), stamp (Seconds)
  ↓ Pipeline::pushGnss() -> m_gnssQueue -> runStep()

[当前严重不足]:
  - 仅 ECEF 位置, 无 Doppler 速度
  - 无伪距/载波相位原始测量 (IC-GVINS/OB_GINS 同级浅耦合)
  - 无 HDOP/VDOP, 无星座信息

[推荐扩展结构]:
  struct GnssMeasurement {
    Seconds stamp;
    Vector3d positionEcef;    // ECEF
    Vector3d positionEnu;     // 局部 ENU (站心原点转换后)
    Vector3d velocityEcef;    // Doppler
    Vector3d stdPosition;     // 位置标准差 (m)
    bool positionFixed;       // RTK fix
    uint8_t numSatellites;    // 可用星数
  };
  // 坐标转换: 需引入 Earth 类 (参考 IC-GVINS/OB_GINS)
  // GNSS 因子: e = pose.t + R(q)*lever - gnss_ENU; W = diag(1/sigma^2)
```

### 9.4 视觉数据管线 (多相机抽象)

```
相机硬件 (单目/双目/N目, CameraRig 统一)
  ↓ (VisualObservation: types.hpp:21-25)
cameraId (uint32_t), stamp (Seconds), featureCount (int) --- 仅计数!
  ↓ Pipeline::pushVisualFrame() (pipeline.cpp:21-28):
    for obs in frame: validateCameraId(obs.cameraId) -> pushVisual(obs)
  ↓ runStep() -> backend.onStep(..., visual_queue.size(), ...)

[CameraRig 外参] (camera_rig.hpp/cpp):
  CameraRig(vector<Isometry3d> bodyFromCamera)    // IMU->相机
  monocular(bodyFromCam0) -> N=1
  bodyFromCamera(cameraId) -> const Isometry3d&

[当前严重不足]:
  - featureCount 仅 int 类型 -> 无法做重投影
  - 无特征坐标, 无描述子, 无特征ID
  - CameraRig 无内参 (fx,fy,cx,cy,distortion)

[推荐扩展]:
  struct Feature {
    Eigen::Vector2d uv_raw, uv_norm; // 原始/去畸变归一化坐标
    size_t feat_id; float response;  // ID + 响应
  };
  struct VisualObservation {
    uint32_t cameraId; Seconds stamp;
    std::vector<Feature> features;  // 替换 featureCount
  };
  // CameraRig 需扩展: 内参 + td + 外参可优化标记
```

### 9.5 Pipeline 数据流 (当前实现)

```
Pipeline (pipeline.hpp + pipeline.cpp):
  pushImu()    -> deque<ImuMeasurement>    (max 4096)
  pushGnss()   -> deque<GnssMeasurement>   (max 4096)
  pushVisual() -> deque<VisualObservation> (max 4096)
  pushVisualFrame() -> 逐 cameraId 展开入队

trimQueues(): size > 4096 -> pop_front() 丢弃最旧
runStep():
  1. backend->onStep(tick, imu.size(), visual.size(), gnss.size())
  2. 清空所有 3 个队列
  3. tick++

设计缺陷 (详见 7.2):
  onStep 传 计数 而非 实际数据 -> 后端无法访问测量值
  修复: onStep(span<const ImuMeasurement>, span<const VisualObservation>, ...)
```

### 9.6 传感器接口层

```
(sensor_interfaces.hpp:1-33)

IImuIntegrator::integrate(m): 积分单个 IMU -> 需 output()+reset()
IGnssHandler::handle(m): ECEF->ENU, 异常检测 -> 需质量回调
IVisualFrontend::onObservation(obs) / onSyncFrame(frame):
  上层实现特征匹配+三角化+关键帧
  -> 需匹配结果回调、关键帧判定接口
```

### 9.7 跨传感器协同 (当前 vs phad_fusion 计划)

| 机制 | 当前 (slam_fusion_core) | phad_fusion 计划 |
|------|------------------------|-----------------|
| **时间同步** | 无 (stamp 仅占位) | IMU-CAM td 在线估计 |
| **数据缓冲** | deque + runStep 清空 | 双缓冲 + 时间窗口管理 |
| **初始化** | 无 | GNSS+IMU 重力对齐 (参考 IC-GVINS) |
| **多相机同步** | VisualFrame N>=1 | 帧间特征匹配 |
| **降级** | 无 | GNSS->IMU->恒速 (参考 SuperOdom) |
| **线程安全** | 无锁 (deque) | mutex/lock-free 保护 |
| **后端** | IOptimizationBackend (占位) | GTSAM ISAM2 (增量因子图) |

### 9.8 完整管线规划图

```
slam_fusion_core (设施层)         phad_fusion (实现层)
──────────────────────────       ──────────────────────
ImuMeasurement + stamp           + noiseModel, bias, cov
  ↓                                ↓
Pipeline::pushImu()              ROS2 Imu sub -> convert
Pipeline::runStep()              GTSAM PreintegrationFactor
  ↓                                ↓
IImuIntegrator (接口)            GTSAM integrator impl
  ↓                                ↓
IOptimizationBackend::onStep()   GTSAM ISAM2 factor graph
  - 传入 actual data (修复)      - IMU/GNSS/visual factors
  - tick 调度                    - incremental optimize
                                  ↓
                                pose graph + maps
```