...
| 传感器 | 频率 | 数据格式 | 接口 | 负责模块 |
|--------|------|----------|------|----------|
| LiDAR (Livox Avia) | 10 Hz | `livox_ros_driver2::msg::CustomMsg` | `slam.cc:106-114` | PointCloudPreprocess |
| LiDAR (Velodyne 32) | 10 Hz | `sensor_msgs::PointCloud2` | `slam.cc:106-114` | PointCloudPreprocess |
| LiDAR (Ouster 64) | 10 Hz | `sensor_msgs::PointCloud2` | `slam.cc:106-114` | PointCloudPreprocess |
| LiDAR (RoboSense) | 10 Hz | `sensor_msgs::PointCloud2` | `slam.cc:106-114` | PointCloudPreprocess |
| IMU (9-axis) | ≥200 Hz | `sensor_msgs::Imu` | `slam.cc:94-104` | LaserMapping → IESKF |

### 8.2 LiDAR 管线

#### 8.2.1 原始数据

- **Livox Avia** (`lidar_type=1`)：非重复扫描（FOV 70.4°×77.2°），CustomMsg → `PointCloudPreprocess::Process(CustomMsg)`
- **Velodyne 32** (`lidar_type=2`)：`PointCloudPreprocess::VelodyneHandler()` → PCL `PointXYZIT`
- **Ouster 64** (`lidar_type=3`)：`PointCloudPreprocess::Oust64Handler()` → 含 ROI 高度过滤
- **RoboSense** (`lidar_type=4`)：`PointCloudPreprocess::RoboSenseHandler()`
- 自定义点格式 `PointXYZIT`（`point_def.h:17-23`）：`x, y, z, intensity, time`
- 配置 topic：`lidar_topic: "points_raw"`, `livox_lidar_topic: "/livox/lidar"`

#### 8.2.2 预处理

**均匀采样**（`pointcloud_preprocess.h:59`）：
- `point_filter_num_`（默认 1 = 不采样）：每 N 个点保留 1 个
- Livox: `i % point_filter_num_ == 0`（`pointcloud_preprocess.cc:54`）

**距离盲区过滤**（各 handler 中 `blind_` 检查）：
- Livox: `pointcloud_preprocess.cc:63-68`
- Velodyne: `pointcloud_preprocess.cc:242`

**ROI 高度过滤**（Ouster）：`pointcloud_preprocess.cc:108-109`

#### 8.2.3 去畸变（IMU 后向传播）

`imu_processing.hpp:173-313` `ImuProcess::UndistortPcl()`：

**步骤 1：前向传播**（Forward Propagation）：
- 从 LiDAR 扫描开始时间 `pcl_beg_time` 到结束时间 `pcl_end_time`
- 在每个 IMU 测量点之间递推 ESKF（预测）
- 记录每帧 IMU 的 pose：`imu_pose_.emplace_back(state.rot, state.pos, 0, aT, gT, vel)`（`imu_processing.hpp:247-249`）

**步骤 2：点云时间排序**（`imu_processing.hpp:263-264`）：
```cpp
std::sort(pcd_out->points.begin(), pcd_out->points.end(),
          [](const PointType &p1, const PointType &p2) { return p1.time < p2.time; });
```

**步骤 3：后向补偿**（`imu_processing.hpp:271-312`）：
- 从最后一帧 IMU pose 往前遍历
- 对每个 LiDAR 点按其 `offset_time` 插值 IMU pose 补偿：
  ```cpp
  p_compensate = imu_state.offset_R_lidar⁻¹ *
                 (imu_state.rot⁻¹ *
                  (R_i * (imu_state.offset_R_lidar * P_i + imu_state.offset_t_lidar) + T_ei) -
                  imu_state.offset_t_lidar);
  ```
- 补偿方向是**帧运动方向的逆方向**（`imu_processing.hpp:290-292`）

#### 8.2.4 Voxel Grid 降采样

`laser_mapping.cc:222-229`：
```cpp
voxel_scan_.setInputCloud(scan_undistort_);
voxel_scan_.filter(*scan_down_body_);
```
降采样尺寸：`fasterlio.filter_size_scan`（NCLT 默认 0.5m, `default_nclt.yaml:18`）

**自保护机制**（`laser_mapping.cc:232-239`）：
若降采样后点数 < 原始点云的 10%，用更小体素尺寸 0.1m 重新降采样。

#### 8.2.5 特征提取（在线平面拟合）

**不使用传统 edge/planar 分类**。采用统一的在线平面拟合策略：

1. 在 IVox3d 增量 voxel 地图中查找 `NUM_MATCH_POINTS = 5` 个最近邻点（`laser_mapping.cc:583`）：
   ```cpp
   ivox_->GetClosestPoint(point_world, points_near, 5);
   ```

2. 至少需要 `MIN_NUM_MATCH_POINTS = 3` 个匹配点（`laser_mapping.cc:584`）

3. **在线平面拟合**（`laser_mapping.cc:586-587`）：
   ```cpp
   point_selected_surf_[i] = math::esti_plane(plane_coef_[i], points_near, 0.1);
   ```
   平面估计函数（`lightning_math.hpp:358-414`）：
   - 约束 `n^T * p = -1`，`A.colPivHouseholderQr().solve(b)` 求解法向量
   - 5 个点时用固定 5×3 矩阵；更多点用动态矩阵
   - 输出归一化平面系数 `(a, b, c, d)` 满足 `a*x + b*y + c*z + d = 0`
   - 验证**所有**邻点到平面距离 ≤ `ESTI_PLANE_THRESHOLD = 0.1`

4. **距离有效性验证**（`laser_mapping.cc:595-596`）：
   ```cpp
   bool valid_corr = p_body.norm() > 81 * pd2 * pd2;
   ```
   等效于 `d_body > 9 * |plane_residual|`

5. 有效匹配的**点到平面残差**：`residuals_[i] = pd2`

#### 8.2.6 匹配/配准（IESKF + Cauchy 鲁棒核）

**IESKF 架构**（`eskf.cc:102-344` Update 步骤）：
- 状态向量 23 维名义状态 / 24 维误差状态：
  `[pos(3), rot(SO3), offset_R_lidar(SO3), offset_t_lidar(3), vel(3), bg(3), ba(3), grav(S2)]`
- **迭代循环**（`eskf.cc:272-344`）：`i = -1` 到 `maximum_iter_`（NCLT 默认 4 次）
  - `i = -1`：初始猜测计算 `init_res` 作为归一化基准
- 卡尔曼增益：
  - 观测维度 > 状态维度：标准 `K = P*H^T*(H*P*H^T+R)⁻¹`
  - 观测维度 ≤ 状态维度（典型 LiDAR 情况）：`K = (P/R + H^T*H)⁻¹ * H^T`
- 状态更新：`dx = K*r + (K*H - I) * dx_current`
- 收敛检查：`max(|dx|) < 1e-3`

**观测雅可比**（`laser_mapping.cc:629-684`）：12 维（位置 3 + 旋转 3 + 外参旋转 3 + 外参平移 3）
```cpp
C = Rt * norm_vec          // map frame 法向量转到 IMU frame
A = point_crossmat * C     // 旋转雅可比
H.row = [norm | A | B(if extrinsic) | C(if extrinsic)]  // 1×12
```

**Cauchy 鲁棒核**（`laser_mapping.cc:663-676`）：
```
ρ(x) = delta² * ln(1 + x/delta²)    // delta = 2.0
dρ/dx = 1 / (1 + x/delta²)          // 作为权重乘入 H 矩阵
```

**残差统计**（`laser_mapping.cc:686-700`）：使用**残差中位数平方**作为收敛度量（`lidar_residual_mean_`）

**Anderson Acceleration**（`eskf.cc:254-261`，可选，默认关闭 `use_aa: false`）：
```cpp
new_dx_all = aa_.compute(dx_all);
x_ = start_x.boxplus(new_dx_all);
```

#### 8.2.7 算法消费（IMU 递推 + PGO）

**IMU 前向递推**（`nav_state.h:59-111`）：
- 不使用预积分（pre-integration）
- 欧拉离散积分：
  ```cpp
  pos += vel * dt
  rot = rot * SO3::exp((gyro - bg) * dt)
  vel += (rot * (acc - ba) + grav) * dt
  ```
- 过程噪声：`Q = diag(gyr_cov²(0.1), acc_cov²(0.1), b_gyr_cov²(0.0001), b_acc_cov²(0.0001))`

**外参在线估计（可选）**：`extrinsic_est_en_` 开关控制，关闭时 H 矩阵对应列置零（`laser_mapping.cc:654-659`）

**关键帧选取**（`laser_mapping.cc:280-293`）：
- 距离 > `kf_dis_th_`（NCLT: 3.0m）
- 旋转 > `kf_angle_th_`（NCLT: 15°）

### 8.3 IVox3d 地图管线

`ivox3d.h:37-105` `IVox<3, IVoxNodeType::DEFAULT, PointType>`：

```
数据结构：std::unordered_map<KeyType, ...> grids_map_
容量：capacity_ = 1,000,000 grids（LRU 淘汰）
NCLT 配置：ivox_grid_resolution = 0.5m, nearby_type = 18
```

**IVoxNode**（`ivox3d_node.hpp:13-99`）：每节点存储最多 10 个点（LRU 插入策略）

**地图更新**（`laser_mapping.cc:488-547` `MapIncremental()`）：
- 并行 `std::for_each` 更新
- 新点距离 voxel 中心比现有点更远且未达容量时才添加

### 8.4 回环检测与 PGO 管线

#### 8.4.1 候选检测

`loop_closing.cc:96-143` `DetectLoopCandidates()`：
- 欧氏距离筛选：`|kf_opt_pose - hist_kf_opt_pose|_xy < max_range`（NCLT: 60m）
- ID 间隔 > 20 帧

#### 8.4.2 Submap 构建

`loop_closing.cc:173-203`：
- 候选帧 ±40 帧构建 submap（每隔 4 帧取 1 帧，约 20 帧）
- 使用 `OptPose` 转到世界坐标系
- Source 端仅用当前帧

#### 8.4.3 多分辨率 NDT 匹配

`loop_closing.cc:218-238`：
```
分辨率层级: r ∈ {10.0, 5.0, 2.0, 1.0} 米
每层：
    target = VoxelGrid(submap_kf1, r*0.1)
    source = VoxelGrid(submap_kf2, r*0.1)
    ndt.setResolution(r)
    ndt.align(source, target, initial_guess)
    下一层初始猜测 = 当前层对齐结果
```
得分筛选：`c.ndt_score_ > ndt_score_th_`（NCLT: 1.0）— 实际是 `getTransformationProbability()`

#### 8.4.4 miao PGO 优化

`loop_closing.cc:28-54` `LoopClosing::Init()`：
```
miao::OptimizerConfig(LEVENBERG_MARQUARDT, LINEAR_SOLVER_SPARSE_EIGEN, false)
config.incremental_mode_ = true
optimizer_ = SetupOptimizer<6, 3>(config)
```

**因子图结构**（`loop_closing.cc:253-350` `PoseOptimization()`）：

| 约束类型 | 测量 | 噪声 (trans / rot) |
|---------|------|-------------------|
| 运动约束 | LIO odom between (kf-3 到 kf) | 0.1m / 3° |
| 高度先验（可选） | z = 0 | 0.1m |
| 回环约束 | NDT Tij | 0.2m / 3° |

Cauchy 鲁棒核（delta = 1.04）用于回环边。
外点剔除：`Chi²(edge) > Delta(kernel)` → 屏蔽该边（`SetLevel(1)`）。

最大迭代 20 次。优化后更新所有关键帧 `opt_pose_`。

### 8.5 时间同步

**LiDAR-IMU 同步**（`laser_mapping.cc:422-486` `SyncPackages()`）：

1. **计算扫描结束时间**：
   - 若点云最后点 `time > 0.5 * lidar_mean_scantime_`：`scan_end = begin + last_point.time/1000`
   - 否则用默认 `lo::lidar_time_interval = 0.1s`
   - 异常保护：若 > 5*lidar_time_interval，截断

2. **等待条件**：IMU 缓冲区最新时间戳 > `lidar_end_time_`

3. 取出扫描开始到结束之间的所有 IMU（`laser_mapping.cc:464-476`），打包到 `MeasureGroup measures_`

### 8.6 初始化

**IMU 初始化**（`imu_processing.hpp:120-171` `IMUInit()`）：
- 在线递推式均值/协方差估计（Welford 算法），**最多 20 帧 IMU**
- 达到 20 帧后：
  ```cpp
  grav = S2(-mean_acc / |mean_acc| * 9.81)
  bg = mean_gyr          // 初始 gyro bias
  ba = Vec3d::Zero()     // acc bias 初始为零
  cov_acc *= (9.81 / |mean_acc|)²  // 加速度协方差重缩放
  ```
- **不需要静止状态**，运动中即可完成

**首帧点云**（`laser_mapping.cc:180-194`）：
- 不执行 IESKF 更新
- IMU 初始化状态直接设为 `state_point_`
- 点云全部加入 IVox3d 地图
- IESKF 在 `(curr_time - first_lidar_time) ≥ 0.1s` 后才标记为 `inited`

### 8.7 降级

- 有效特征点 < 1：跳过当前帧（`laser_mapping.cc:623-627`）
- 降采样后 < 10% 原点：体素尺寸退化到 0.1m
- 无回环：里程计累积漂移
- 对称/重复结构：欧氏距离回环检测可能失效（无 ScanContext 补充）
- 多楼层场景：高度约束需谨慎关闭
