...
|--------|------|----------|-------------|----------|
| 相机 (mono/stereo) | 10-30 Hz | `sensor_msgs::Image` → `cv::Mat` | `IMAGE0_TOPIC`, `IMAGE1_TOPIC` | `FeatureTracker::trackImage` |
| IMU | 100-200 Hz | `acc[3]` [m/s²], `gyr[3]` [rad/s] | `IMU_TOPIC` → `imu_callback` | `Estimator::processIMU`, `IntegrationBase` |
| GPS (可选) | 1-10 Hz | `sensor_msgs::NavSatFix` (lat/lon/alt) | `GPS_TOPIC` | `global_fusion::GlobalOptimization` |

### 8.2 相机数据处理管线

#### 原始数据
- **图像格式**: 灰度图 `cv::Mat` (CV_8UC1)，色彩转换由 ROS 接口完成
- **分辨率**: `W×H` 像素，由相机模型定义
- **标定来源**: camodocal 相机模型 (`camera_models/include/camodocal/`)，支持 Pinhole/MEI/Equidistant/CataCamera，从 YAML 加载
- **外参**: `T_IC` = `[RIC, TIC]` (IMU→Camera, 4×4)，`Estimator::setParameter()` 中设定，支持在线估计旋转 (`ESTIMATE_EXTRINSIC=2`)
- **时间偏移**: `td` (IMU→Camera 延迟)，`setParameter()` 中设定，支持在线估计 (`ESTIMATE_TD=true`)

#### 前端处理 (`FeatureTracker::trackImage`)
`vins_estimator/src/featureTracker/feature_tracker.cpp:94-306`:

**步骤 1: KLT 光流跟踪** (line 119-146)
- `cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, ...)`:
  - 参数: `winSize=21×21`, 1层(带预测)/3层(无预测), `OPTFLOW_USE_INITIAL_FLOW`
- 视觉预测 (`setPrediction`): 从 Estimator 传入预测 3D 点 → `spaceToPlane` 投影回像素 → 作为 LK 初始猜测
- 若成功点 < 10: 回退到 3 层无预测金字塔
- **反向光流** (`FLOW_BACK=true`): `cur_pts → reverse_pts`，过滤条件: `dist(forward,reverse) < 0.5px`
- 边界检查: `inBorder(cur_pts[i])`

**步骤 2: 特征管理** (`setMask`, line 55-83)
- `track_cnt` 全部 +1 (标记跟踪帧数)
- 按 `track_cnt` 排序 → 优先保留长期跟踪点
- 距离掩码: `MIN_DIST` 半径内冲突点丢弃

**步骤 3: 新特征提取** (line 172-185)
- `cv::goodFeaturesToTrack(cur_img, new_pts, MAX_CNT-tracked_num, 0.01, MIN_DIST)` (Shi-Tomasi 角点)
- 补全到 `MAX_CNT` 个特征
- 新点 `track_cnt=1`, 递增 `n_id` 全局 ID

**步骤 4: 去畸变与速度** (line 187-199)
- `undistortedPts()`: `cam->liftProjective(pt)` 去畸变到归一化坐标 (camodocal)
- `ptsVelocity()`: 计算归一化平面运动速度 `v = (pt_cur - pt_prev)/dt`，用于时间偏移 TD 因子校正

**步骤 5: 立体匹配** (line 202-243, STEREO 模式)
- 左目→右目 `cv::calcOpticalFlowPyrLK(cur_img, rightImg, ...)`
- `FLOW_BACK` 反向验证
- `undistortedPts(right)` + `ptsVelocity(right)`

**步骤 6: RANSAC 剔除** (被 `#if 1` 禁用，line 167-170)
- `rejectWithF`: 去畸变坐标 → `cv::findFundamentalMat(ll, rr, CV_FM_RANSAC, F_THRESHOLD, 0.99)`

**步骤 7: 组装特征帧**
- 每个 feature: `[x, y, z, p_u, p_v, velocity_x, velocity_y, (right_u, right_v)]`
- 推入 `featureBuf` → 后端消费

### 8.3 IMU 数据处理管线

#### 原始数据
- **测量**: `acc` [m/s²] 3D, `gyr` [rad/s] 3D, `dt` [s]
- **噪声参数** (YAML): `acc_n` (加速度计噪声密度), `gyr_n` (陀螺仪噪声密度), `acc_w` (加速度计随机游走), `gyr_w` (陀螺仪随机游走)
- **重力**: `G = [0, 0, -9.81]` m/s² (配置)

#### 接收与缓冲
`imu_callback` (`rosNodeTest.cpp:135-148`):
- `estimator.inputIMU(t, acc, gyr)`
- 推入 `accBuf` / `gyrBuf` (deque)
- 若系统进入 `NON_LINEAR` → 立即 `fastPredictIMU()` 高频传播

#### IMU 预积分 (`IntegrationBase::midPointIntegration`)
`vins_estimator/src/factor/integration_base.h:63-137`:

**中值积分** (每段相邻 IMU 测量):
```
un_acc_0 = δq * (acc_0 - linearized_ba)       // 旋转到起始帧
un_gyr = 0.5*(gyr_0 + gyr_1) - linearized_bg   // 角速度中值
δq = δq * Quaternion(1, un_gyr(0)*dt/2, ...)    // 四元数更新
un_acc_1 = δq_new * (acc_1 - linearized_ba)
un_acc = 0.5*(un_acc_0 + un_acc_1)              // 加速度中值
δp = δp + δv*dt + 0.5*un_acc*dt*dt             // 位置预积分
δv = δv + un_acc*dt                              // 速度预积分
```

**误差状态传播** (15 维: δp, δθ, δv, δb_a, δb_g):
- 状态转移矩阵 F(15×15): 包含旋转/加速度/偏置交叉项
- 噪声矩阵 V(15×18): `noise = diag(ACC_N²·I₃, GYR_N²·I₃, ACC_N²·I₃, GYR_N²·I₃, ACC_W²·I₃, GYR_W²·I₃)`
- 协方差: `P = F·P·F^T + V·Q·V^T`
- Jacobian: `J = F·J` (对偏置的导数，用于一阶偏置更新)

#### 状态传播 (`Estimator::processIMU`)
`estimator.cpp:375-409`:
- 调用 `pre_integrations[j]->push_back(dt, acc, gyr)` 累积 IMU 因子
- 中值积分更新 `Rs[j], Ps[j], Vs[j]` (前端跟踪预测)
- `tmp_pre_integration`: 下一帧准备的预积分对象

#### 快速预测 (`Estimator::fastPredictIMU`)
`estimator.cpp:1571-1584`:
- 进入 NON_LINEAR 后，每次 IMU 回调做中值积分预测
- 推送 `latest_Q, latest_P, latest_V` 发布高频里程计

`Estimator::getIMUInterval(prevTime, curTime)` (`estimator.cpp:227-260`):
- 取 `[t0, t1]` 区间 IMU，丢弃 t0 前，保留最后一个 ≤t0，额外多加最后一帧作插值端点
...
```
15 维残差 (经过 sqrt_info 白化):
r_P = Q_i^{-1}*(P_j - P_i - V_i*Δt + 0.5*g*Δt²) - corrected_δp     [3维]
r_R = 2*(corrected_δq^{-1} * (Q_i^{-1}*Q_j)).vec()                   [3维]
r_V = Q_i^{-1}*(V_j - V_i + g*Δt) - corrected_δv                     [3维]
r_Ba = Ba_j - Ba_i                                                    [3维]
r_Bg = Bg_j - Bg_i                                                    [3维]

偏置一阶校正:
corrected_δq = δq * exp(J_R_bg * dbg)
corrected_δv = δv + J_V_ba*dba + J_V_bg*dbg
corrected_δp = δp + J_P_ba*dba + J_P_bg*dbg
```
- 优化变量: `[Pose_i(7), VelBias_i(9), Pose_j(7), VelBias_j(9)]`
- 信息矩阵: 预积分协方差逆 `P^{-1}` (15×15)
- 跳过 `sum_dt > 10.0` 的预积分

#### 视觉重投影因子 (`ProjectionTwoFrameOneCamFactor`)
`vins_estimator/src/factor/projectionTwoFrameOneCamFactor.cpp:43-150`:

**逐观测逆深度投影** (2 维残差):
```
pts_i_td = pts_i - (td - td_i)*velocity_i          // TD 一阶校正
pts_j_td = pts_j - (td - td_j)*velocity_j
P_cam_i = pts_i_td / inv_depth                      // 逆深度恢复 3D
P_IMU = qic*P_cam_i + tic                           // camera→IMU
P_W = Qi*P_IMU + Pi                                  // IMU→World
P_IMU_j = Qj^{-1}*(P_W - Pj)                        // World→IMU_j
P_cam_j = qic^{-1}*(P_IMU_j - tic)                  // IMU→camera
r = (P_cam_j / z_j).head<2>() - pts_j_td.head<2>()  // 归一化平面坐标差
```
- 优化变量: `[Pose_i(7), Pose_j(7), Ex_Pose(7), inv_dep(1), Td(1)]`
- 信息矩阵: `sqrt_info = FOCAL_LENGTH/1.5 * I_{2×2}`

#### 立体因子
- `ProjectionOneFrameTwoCamFactor`: 纯静态立体约束 (同帧左右目)，不含位姿变量
- `ProjectionTwoFrameTwoCamFactor`: 时序+立体联合约束 (不同帧相机 0 和相机 1)

#### 边缘化先验因子 (`MarginalizationFactor`)
`vins_estimator/src/factor/marginalization_factor.cpp:347-395`:
```
dx = x - x_linearization_point
residual = linearized_residuals + linearized_jacobians * dx
```
- Schur 补: `A' = A_rr - A_rm*A_mm^{-1}*A_mr`, `b' = b_r - A_rm*A_mm^{-1}*b_m`
- `A_mm` 求逆: 特征值分解 → 伪逆 (截断 `λ < eps`)

### 8.5 跨传感器协同

#### 时间同步
- `sync_process` 线程: STEREO 模式下 ±3ms 容差对齐左右图像时间戳
- `td` (时间偏移) 在线估计: `ProjectionTwoFrameOneCamFactor` 中利用特征速度做一阶校正
- IMU-图像窗口: `getIMUInterval` 基于图像时间戳切分 IMU 数据

#### 数据缓冲策略
- IMU: `accBuf` / `gyrBuf` (deque), 2000 队列长度
- 特征: `featureBuf` (deque), 前端输出→后端消费
- 多线程: `MULTIPLE_THREAD=true` 时 `processMeasurements` 独立线程，每 2 帧处理 1 帧

#### 初始化管线
**单目+IMU** (`Estimator::initialStructure` + `visualInitialAlign`):
1. `relativePose`: 寻找视差>30/f 的参考帧 → 5 点法 F + `cv::recoverPose`
2. `GlobalSFM::construct`: 全局 SfM (PnP + 三角化 + Full BA Ceres, 0.2s 限时)
3. `solveGyroscopeBias`: `A δ_bg = b`, 从视觉旋转 vs IMU 预积分旋转差异估计
4. `LinearAlignment`: 线性系统估计速度 + 重力 + 尺度 (LDLT)
5. `RefineGravity`: 切平面 2D 参数化，迭代 4 次收敛

**双目+IMU**: PnP → 三角化 → 陀螺仪偏置 → 滑动窗口优化 → NON_LINEAR

#### 降级与异常处理
- Huber 核: `HuberLoss(1.0)` 用于所有视觉因子
- 重投影离群剔除: `outliersRejection`: `ave_err*FOCAL_LENGTH > 3` → 标记离群
- 偏置重传播: `IntegrationBase::repropagate`: 偏置显著变化时从原始数据重积分 (当前被 `#if 0` 禁用)
- VIO→Loop→GPS 松耦合: 回环信息不注入 VIO 窗口，GPS 因子通过位姿图独立优化
- 边缘化策略: 视差足够 → `MARGIN_OLD`; 视差不足 → `MARGIN_SECOND_NEW`