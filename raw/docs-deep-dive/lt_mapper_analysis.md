...
| 数据源 | 格式 | 来源 | 内容 |
|--------|------|------|------|
| **关键帧点云** | PCD (`pcl::PointXYZI`) | 外部前端 SLAM 生成 | 每帧 LiDAR 关键帧 (10-20Hz 降采样) |
| **Scan Context** | SCD 二进制 | 外部前端 SLAM 生成 | 每帧 SC 描述子 (20扇区x40环 polar matrix) |
| **位姿图** | TXT (`optimized_poses.txt`) | 外部前端 SLAM 生成 | 节点索引 + 位姿 (Tx,Ty,Tz,Qx,Qy,Qz,Qw) |
| **回环边** | TXT (位姿图内嵌) | 外部前端 SLAM 生成 | 会话内部回环约束 (from->to 相对 Pose3) |

### 9.2 会话数据结构 (输入层)

```
前端 SLAM (SC-LIO-SAM / SC-A-LOAM / FAST_LIO_SLAM) 输出:
  session_dir/
  ├── keyframes/            # PCD 点云 (逐帧 x,y,z,intensity)
  ├── scd/                  # Scan Context 描述符 (二进制)
  └── optimized_poses.txt   # 行: index x y z qx qy qz qw

Session 加载 (Session.cpp):
  ↓ loadSessionGraph():
    解析 optimized_poses.txt -> nodes_(multimap: from_idx -> Node{idx, initial})
    边分类: |from - to| == 1 -> odometry edge; 其他 -> loop edge
  ↓ loadSessionScanContextDescriptors():
    解析 scd/ 目录 -> scManager.polarcontexts_ (20扇区x40环 矩阵)
    附: ring_key (径向编码), sector_key (扇区编码)
  ↓ loadSessionKeyframePointclouds():
    解析 keyframes/ PCD -> cloudKeyFrames[i] (体素降采样后的关键帧)
  ↓ 体素滤波: downSizeFilterICP.setLeafSize(config) (VoxelGrid)
  ↓ 坐标系: 每个 Session 独立局部坐标系
```

### 9.3 多会话位姿图管线 (GTSAM ISAM2)

```
主流程 (LTslam.cpp:79-98, LTslam::run()):
[初始化] initOptimizer() + initNoiseConstants():
  ↓ ISAM2: relinearizeThreshold=0.1, relinearizeSkip=1
  ↓ 噪声模型:
    priorNoise:  1e-12 (全部6维) -> anchor 强先验 (固定 base session)
    odomNoise:   1e-4  (全部)    -> 单会话内里程计边
    loopNoise:   1e-4(旋转), 1e-3(位移) -> 单会话内部回环边
    largeNoise:  pi^2(旋转), 1e8(位移) -> 非中心会话 anchor 弱先验
    robustNoise: Cauchy M-estimator + 0.5 (方差) -> 跨会话回环边
[图构建] addAllSessionsToGraph():
  ↓ 各 Session 的 nodes_ + edges_ -> ISAM2 graph
  ↓ anchor 节点: genAnchorNodeIdx(session_idx) -> 每会话一个 anchor
  ↓ base session: priorNoise (强约束, 固定原点)
  ↓ query session: largeNoise (弱约束, 允许大范围漂移)
  ↓ anchor 初值: poseOrigin (单位 Pose3, 即 identity)
[优化] optimizeMultisesseionGraph():
  ↓ isam->update(graph, initialEstimate)
  ↓ 5x isam->update() (多次更新确保收敛)
  ↓ isamCurrentEstimate = isam->calculateEstimate()
```

### 9.4 Anchor 节点机制 (坐标变换链路)

```
核心因子: BetweenFactorWithAnchoring<Pose3> (BetweenFactorWithAnchoring.h:18-127)

误差计算 (evaluateError, line 86-100):
  hx1 = anchor_p1 (+) p1    // 局部坐标 + anchor 变换 = 中心坐标
  hx2 = anchor_p2 (+) p2    // anchor_p2 也允许漂移
  hx  = hx1 (-) hx2         // 中心坐标下的相对位姿
  e   = Log(measured^(-1) * hx)  // 6维 tangent space 误差

四变量因子 (key1, key2, anchor_key1, anchor_key2):
  对 4 个变量均解析计算雅可比
  使用 GTSAM traits<T>::Compose / traits<T>::Between 自动推导

优势: 避免逐步ICP配准漂移累积; 所有会话在统一中心坐标系联合优化
```

### 9.5 回环检测管线 (SC + RS 双模式)

```
[SC 回环] detectInterSessionSCloops() (LTslam.cpp:304-333):
  ↓ 遍历 query session 的所有 SC 描述符
  ↓ detectLoopClosureIDBetweenSession():
    Ring Key KD 树检索 -> 3 个最近邻 (NUM_CANDIDATES_FROM_TREE=3)
    V-key 快速对齐 (fastAlignUsingVkey) -> 最佳列偏移
    余弦距离计算 (distDirectSC, +/-5% 搜索范围)
    阈值: SC_DIST_THRES (0.5)
  ↓ ICP 验证 (doICPVirtualRelative, LTslam.cpp:187-242):
    PCL ICP 在局部坐标系, 搜索半径 150m, 最大 100 次迭代
    fitness score 阈值过滤
    等间隔采样: equisampleElements() -> 最多 kNumSCLoopsUpperBound 个
  ↓ 添加 SC 回环边 -> ISAM2 优化

[RS 回环] findNearestRSLoopsTargetNodeIdx() (LTslam.cpp:450-505):
  ↓ SC 未匹配的节点 -> 半径搜索 (10m)
  ↓ 信息增益选择最优匹配:
    S = Sy + H1*Cov1*H1^T + H2*Cov2*H2^T   (后验信息矩阵)
    IG = 0.5 * log(det(S) / det(Sy))       (信息增益, 越大越好)
  ↓ ICP 验证 (doICPGlobalRelative, LTslam.cpp:245-301):
    PCL ICP 在中心坐标系, 采样策略同上
  ↓ 添加 RS 回环边 -> ISAM2 优化
```
...
```
LT-removert 模块 (Removerter.cpp):
  输入: LT-SLAM 对齐位姿 (中心坐标系) + 原始会话点云
  ↓ Range Image 投影:
    点云 -> spherical projection (360度全景 range image)
  ↓ 高动态移除:
    逐帧 range image -> occupancy 状态比较
    前一帧有占据、后一帧消失 -> dynamic (移动物体)
  ↓ 低动态/变化检测:
    跨会话比较 range image -> 新增/消失区域
    生成 Delta Map (差异 map, 用于增量更新)
...
| 机制 | 实现 | 参数 |
|------|------|------|
| **时间同步** | 离线文件对齐, 无实时同步需求 | 文件名索引 |
| **坐标框架** | Anchor PGO (局部->中心) | `BetweenFactorWithAnchoring` |
| **前序依赖** | 依赖外部前端 SLAM | 文件 I/O 协议 |
| **初始化** | base session anchor 强先验固定 | `priorNoise=1e-12` |
| **降级** | ICP 失败 -> 跳过 (RS 不添加) | `fitness_score > threshold` |
| **鲁棒性** | Cauchy M-estimator (跨会话回环) | `robustNoise` |

### 9.8 数据流全貌

```
外部前端 (SC-LIO-SAM)
  ↓ keyframes/, scd/, optimized_poses.txt (文件 I/O)
LTslam 后端:
  Session1(中心): anchor(强) + nodes + edges
  Session2(查询): anchor(弱) + nodes + edges
    ↓ ISAM2: 初始图优化
    ↓ SC 回环检测 -> ICP -> 添加回环边 -> ISAM2
    ↓ RS 回环检测 -> 信息增益 -> ICP -> 添加回环边 -> ISAM2
    ↓ 输出: 对齐位姿 (中心坐标系)
LT-removert:
  会话点云 + 对齐位姿
    ↓ Range Image 投影
