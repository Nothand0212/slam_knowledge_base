...
| 传感器 | 频率 | 数据格式 | 接口 | 负责模块 |
|--------|------|----------|------|----------|
| 事件相机 (双传感器立体) | 异步, 百万 events/s | `dvs_msgs::Event` (x,y,ts,polarity) | ROS subscriber: `events_left/right` | `EventQueue` → `TimeSurface` |
| 相机标定 | 一次性 | yaml → `CameraSystem` (K, R, T, dist) | `CameraSystem::readFromYaml()` | `CameraSystem` |
| 同步信号 | 周期性 (由外部触发) | `std_msgs::Empty` or `std_msgs::Float32` | ROS topic: "sync" | `TimeSurface::syncCallback()` |

### 7.2 事件流管线

#### 原始数据
```
事件格式: (x: uint16, y: uint16, ts: ros::Time, polarity: bool)
物理特性:
  - 每个像素独立检测 log(I) 变化, 阈值 ~15% 对比度
  - 分辨率: 346×260 (DAVIS240) or 640×480 (CeleX-V)
  - 噪声: 热噪声 (背景活动 BA), 漏电流噪声
```

#### 预处理
**Step 1 — 事件缓冲** (`utils.h:50-57`):
```
EventQueue = std::deque<dvs_msgs::Event>  (按时间排序队列)
EventBuffer_lower_bound(): 二分查找 O(log N) 定位时间
maxEventQueueLength: 缓冲区最大长度 (默认 N=3,000,000 个事件)
事件异步到达 → push_back() 实时添加
```

**Step 2 — Time Surface 生成** (`TimeSurface.cpp:52-140`):
```
算法: 指数衰减 Time Surface
输入: 同步时间 T + 事件队列
对每个像素 (x,y):
  1. 找最近一次事件 e (t_e ≤ T)
  2. Time Surface 值 = exp(-(T - t_e) / τ)
     其中 τ = decay_ms_ (默认 30ms)
  3. 极性处理 (ignore_polarity_=true):
     TS = exp(-Δt/τ)  (忽略极性)
     或 TS = polarity × exp(-Δt/τ)  (保留极性)
两种模式:
  BACKWARD: 直接计算每个像素
  FORWARD: 先校正畸变再做双线性插值 (避免畸变误差)
```

**Step 3 — 图像平滑** (`TimeSurfaceObservation.h:107`):
```
可选高斯模糊: cv::GaussianBlur(TS, kernelSize, 0.0)
kernalSize 默认 15 (可配置)
减少事件噪声的影响
```

**Step 4 — Negative Time Surface** (`TimeSurfaceObservation.h:118`):
```
TS_negative = 255 - TS  (值域反转)
用于处理 OFF 极性事件的补充信息
```

**Step 5 — 梯度计算** (`TimeSurfaceObservation.h:78-84`):
```
Sobel 算子: cv::Sobel(TS_left, dTS_du, CV_64F, 1, 0)  → dTS_du_left_
            cv::Sobel(TS_left, dTS_dv, CV_64F, 0, 1)  → dTS_dv_left_
```
仅在 `bCalcTsGradient=true` 时计算, 用于 Tracking 的解析雅可比

#### 特征提取 (事件选择)
**Mapping 线程事件筛选** (`esvo_Mapping.cpp:86-103`):
```
createEdgeMask():     仅保留边缘区域事件
createDenoisingMask(): 空间密度去噪 (密度过低区域标记噪声)
extractDenoisedEvents(maxNum=N):
  1. 从事件队列随机采样 maxNum 个事件 (默认 20000)
  2. 通过 mask 过滤 → 保留有效事件
  3. 分配到多线程匹配
```
...
**Tracking 线程点选择**:
```
使用 Mapping 输出的 refPCMap_ (参考 3D 点云)
选取 up to MAX_REGISTRATION_POINTS (默认 500) 个 3D 点
```

#### 匹配与异常值剔除
**立体匹配: 粗+精搜索** (`EventBM.cpp:80-168`):
```
patch_size_X,Y: 25×25 像素 (默认)
视差范围: [min_disparity, max_disparity] = [1, 40]
代价函数: ZNCC (Zero-mean Normalized Cross-Correlation)
匹配流程:
  1. 畸变校正: getRectifiedUndistortedCoordinate(x,y)
  2. 边界检查: coordinates in image
  3. Mask 检查: UndistortRectify_mask_ > 125
  4. 提取模板: left block (25×25)
  5. 有效性检查: 有效像素 > 5% (95% 的 0 值 → 无效)
  6. 粗搜索: 步长 step_ (默认 5), 沿极线 ZNCC 搜索
  7. 精搜索: 在粗搜索最优 ±(step-1) 内逐像素 ZNCC 搜索
阈值: ZNCC_Threshold_ (默认 0.1, 即 ZNCC > 0.9 接受)
```

**Tracking 位姿优化** (`RegProblemLM`):
```
残差: 3D 点投影到 Time Surface, 比较 patch 内 TS 值的差异
解法:
  数值法: Eigen LM (默认)
  解析法: ESVO 论文的 Inverse Compositional 解析雅可比
异常值剔除: Student-t 分布 (td_nu_ 自由度, td_scale_ 尺度)
  |residual| > 3·td_stdvar → 标记 outlier
```

#### 算法消费 — 因子构建
**立体深度: DepthProblem** (`DepthProblem.h`):
```
对每个 EventMatchPair:
  warp 方程: x₁_s, x₂_s = warp(x, d, T_left_virtual)
  残差: r = TS_left(x₁_s) - TS_right(x₂_s)  (TS 一致性)
优化变量: 逆深度 d (1D per event)
损失函数: Student-t (td_nu, td_scale 参数化)
  权重 w = (td_nu + 1) / (td_nu + r²/td_scale²)
```

**Tracking: Registration Problem**:
```
残差向量 r ∈ ℝ^{K·patchSize²} (K 个点, 每个点 patchSize² 残差)
观测模型: TS_cur(π(T·X_ref + δx)) - TS_ref(X_ref)
雅可比:
  解析法: ∂TS/∂u · ∂u/∂X · ∂X/∂ξ  (链式法则)
  数值法: Eigen 自动微分
优化器: Eigen::LevenbergMarquardt (MAX_ITERATION=10, BATCH_SIZE=200)
```
...
DepthFusion::propagate_one_point():
  前帧深度点 → 投影到当前帧 → 生成 propagated depth

DepthFusion::fusion():
  propagated depth + 当前观测 depth → 统计兼容检验
  chiSquareTest(invD1, invD2, var1, var2):
    χ² = (invD1 - invD2)² / (var1 + var2)
    若 χ² > χ²_critical(0.95) → 不兼容 → 不融合
  兼容点: weighted average 更新

DepthRegularization: 空间平滑 (median filter in depth neighborhood)
RegulationRadius_ (默认 5), RegulationMinNeighbours (默认 3)
```
...
```
Time Surface 线程:
  syncCallback 收到同步脉冲 → createTimeSurfaceAtTime()
  → 发布 Time Surface 图像 (time_surface_left/right topic)
Mapping 线程:
  订阅 time_surface_left/right → 立体匹配 → 深度估计
Tracking 线程:
  订阅 pointcloud (Mapping 输出)
  使用 TS_history_ (TimeSurface 历史) → 位姿优化
```

#### 数据缓冲
```
EventQueue: std::deque (FIFO, 最大 3M 事件)
TimeSurfaceHistory: std::map<ros::Time, TimeSurfaceObservation>
  滑动窗口保留最近 K 帧 (TS_HISTORY_LENGTH_, 默认 ~10)
refPCMap_: 映射线程输出的参考点云 (Tracking 等待)
```

#### 初始化管线
```
1. 传感器启动: 累积初始事件 (缓冲建立)
2. 同步脉冲 → 生成首帧 Time Surface
3. Mapping 首帧: 立体匹配建立初版深度地图
4. Tracking 初始化: ESVO_System_Status_ = "INITIALIZATION"
   首帧 identity pose → 进入 WORKING 状态
```

#### 降级策略
```
事件不足:
  ↓ Time Surface 大部分为 0 → 匹配点不足 → 跳过当前帧
Tracking 失败:
  ↓ Registration 残差过大 → 使用恒速模型外推
  ↓ 连续失败 → ESVO_System_Status_ 重置
深度退化:
  ↓ ZNCC < threshold → 不输出深度
  ↓ 跨帧融合 chi-square test → 拒绝 incompatible depth
```
