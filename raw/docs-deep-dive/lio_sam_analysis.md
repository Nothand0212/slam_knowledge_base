...
| 传感器 | 频率 | 数据格式 | 接口 | 负责模块 |
|--------|------|----------|------|----------|
| LiDAR (Velodyne/Ouster) | 10 Hz | `sensor_msgs::PointCloud2` (XYZIRT) | `imageProjection.cpp:96` | ImageProjection |
| LiDAR (Livox) | 10 Hz | `livox_ros_driver::CustomMsg` → 转 `PointCloud2` | `imageProjection.cpp:96` | ImageProjection |
| IMU (9-axis) | ≥200 Hz | `sensor_msgs::Imu` | `imageProjection.cpp:94` + `imuPreintegration.cpp:54` | ImageProjection + imuPreintegration |

### 11.2 LiDAR 管线

#### 11.2.1 原始数据

- **Velodyne**: `VelodynePointXYZIRT`（`imageProjection.cpp:4-15`）：字段 `x, y, z, intensity, ring (uint16), time (float)`。`ring` 字段 0-15（VLP-16）或 0-31（VLP-32）
- **Ouster**: `OusterPointXYZIRT`（`imageProjection.cpp:17-31`）：字段 `x, y, z, intensity, t (uint32, ns), reflectivity, ring (uint8), noise, range`。`t` 字段乘以 `1e-9` 转为秒，转换为 Velodyne 格式（`imageProjection.cpp:214-227`）
- **Livox**: 不支持 range image，使用 `columnIdnCountVec` 顺序填充列索引（`imageProjection.cpp:555-556`）
- **标定来源**：外参 `extRot`（IMU→LiDAR 旋转）和 `extRPY`（IMU→LiDAR RP Y）在 YAML 中配置，`imuConverter()`（`utility.h:252-282`）将 IMU 数据左乘外参旋转矩阵对齐到 LiDAR 坐标系

#### 11.2.2 预处理（去畸变）

**IMU 角速度积分 Deskew**（`imageProjection.cpp:286-519`）：

1. **IMU 队列管理**：`imuHandler`（`imageProjection.cpp:149-172`）回调将 IMU 消息压入 `imuQueue`，经 `imuConverter()` 坐标系对齐
2. **清除过期数据**：丢弃 `timeScanCur - 0.01` 之前的 IMU 数据（`imageProjection.cpp:310-313`）
3. **角速度累积积分**（`imageProjection.cpp:330-362`）：
   ```
   // 零阶保持 + 前向欧拉积分
   imuRotX[cur] = imuRotX[cur-1] + angular_x * dt
   imuRotY[cur] = imuRotY[cur-1] + angular_y * dt  // dt = timeDiff
   imuRotZ[cur] = imuRotZ[cur-1] + angular_z * dt
   ```
4. **逐点校正**（`imageProjection.cpp:489-519`）：
   - 旋转内插：`findRotation()`（`imageProjection.cpp:446-471`）在两 IMU 测量间线性插值
   - 平移 deskew：`findPosition()`（`imageProjection.cpp:473-487`）被注释掉（作者注释"walking speed 下不明显"）
   - 校正目标：`transStartInverse`（`imageProjection.cpp:504`）—所有点统一到**帧起始时刻**坐标系
   ```
   Eigen::Affine3f transBt = transStartInverse * transFinal;
   newPoint = transBt * point;  // 仿射变换
   ```

**里程计 Deskew 辅助**（`imageProjection.cpp:364-444`）：从 `odomQueue`（IMU 预积分增量里程计）提取帧起始/末尾姿态，计算 `odomIncreX/Y/Z`，提供 `initialGuess` 给后续 `mapOptmization` 的 ICP 初始猜测

**必要条件检查**（`imageProjection.cpp:248-281`）：
- `ring` 字段**必须存在**，否则退出（`imageProjection.cpp:261-264`）
- `time` 字段不存在时 deskew 被禁用，发出严重警告（`imageProjection.cpp:280`）

#### 11.2.3 Range Image 投影与降采样

**投影**（`imageProjection.cpp:521-572`）：
- 分辨率：`N_SCAN × Horizon_SCAN`（VLP-16 默认 16×1800，`utility.h:198`）
- 行索引：`ring` 字段
- 列索引：`columnIdn = -round((atan2(x,y) - 90.0°) / 0.2°) + Horizon_SCAN/2`（`imageProjection.cpp:547-551`）
- **降采样**：跳过 `rowIdn % downsampleRate != 0` 的行（`imageProjection.cpp:541`），默认 `downsampleRate=1`
- **范围过滤**：`lidarMinRange ~ lidarMaxRange`（`imageProjection.cpp:534-535`）
- **去重**：同一像素单元保留第一个点（`imageProjection.cpp:562-563`）

#### 11.2.4 特征提取

**曲率计算**（`featureExtraction.cpp:81-101`）：
```
c_i = (Σ_{j=i-5}^{i+5,j≠i} r_j - 10·r_i)²
```
使用 range 值的二阶差分而非几何曲率。边缘点曲率高，平面点曲率低。

**遮挡剔除**（`featureExtraction.cpp:103-139`）：
- 遮挡点：相邻列差 < 10 且 `depth1 - depth2 > 0.3` → 标记 ±5 为不可靠
- 平行光束：`|range[i±1] - range[i]| > 0.02·range[i]` → 标记不可靠

**6 等分区域选取**（`featureExtraction.cpp:141-237`）：
- 每条线分为 6 个子区域：`sp/ep` 通过加权插值计算（`featureExtraction.cpp:156-157`）
- **Corner（边缘）**：曲率 > `edgeThreshold`（0.1），每子区域 ≤20 个，按曲率降序
- **Surface（平面）**：曲率 < `surfThreshold`（0.1），选中后标记 ±5 抑制局部密集
- **体素降采样**：`surfaceCloud` 经 `odometrySurfLeafSize`（0.2m）体素滤波后输出

#### 11.2.5 匹配/配准（Scan-to-Map LM）

**帧-地图 ICP**（`mapOptmization.cpp:1282-1310` `scan2MapOptimization()`）：
- 最多 30 次 LM 迭代
- **Corner 优化**（`mapOptmization.cpp:974-1063`）：5 个最近邻 PCA 判断线特征，点到直线距离残差
- **Surface 优化**（`mapOptmization.cpp:1066-1135`）：5 个最近邻最小二乘平面拟合 `ax+by+cz+d=0`，点到平面距离残差
- **自适应权重**：`s = 1 - 0.9·|pd| / sqrt(||p||)`（类似 Huber 核）
- **退化检测**（`mapOptmization.cpp:1229-1251`）：第一次迭代对 `J^T J` 做特征值分解，特征值 < 100 则投影修正（`matP = matV.inv() * matV2`）
- **收敛条件**：Δrotation < 0.05° 且 Δtranslation < 0.5mm

#### 11.2.6 算法消费（因子图构建）

**① LiDAR 里程计因子**（`imuPreintegration.cpp:378-379`）：
```
PriorFactor<Pose3>(X(key), lidarPose.compose(lidar2Imu), correctionNoise)
```
噪声：正常 `(0.05, 0.05, 0.05, 0.1, 0.1, 0.1)`，退化时 `(1, 1, 1, 1, 1, 1)`

**② 全局里程计 BetweenFactor**（`mapOptmization.cpp:1389-1392`）：
```
BetweenFactor<Pose3>(N-1, N, poseFrom.between(poseTo), odometryNoise)
```
噪声：`(1e-6, 1e-6, 1e-6, 1e-4, 1e-4, 1e-4)`（旋转约束远比平移紧）

**③ GPS PriorFactor**（`mapOptmization.cpp:1465-1469`）：
```
GPSFactor(N, gtsam::Point3(gps), gps_noise)
```

**④ 回环 BetweenFactor**（`mapOptmization.cpp:1477-1495`）：噪声 = `fitness_score.repeated<6>()`

**ISAM2 增量优化**（`mapOptmization.cpp:159-162`）：`relinearizeThreshold=0.1`, `relinearizeSkip=1`。回环闭合后额外 5 次 `update()` 确保收敛。

### 11.3 IMU 管线

**原始数据**：`sensor_msgs::Imu`（加速度 m/s², 角速度 rad/s, 四元数姿态）

**坐标系对齐**：`imuConverter()`（`utility.h:252-282`）左乘外参 `extRot`/`extQRPY`，将 IMU 加速度/角速度/姿态转到 LiDAR 坐标系

**预积分参数**（`imuPreintegration.cpp:214-217`）：
```
accelerometerCovariance = I·σ_acc²  (σ_acc = imuAccNoise)
gyroscopeCovariance     = I·σ_gyr²  (σ_gyr = imuGyrNoise)
integrationCovariance   = I·(1e-4)²
gravity = imuGravity (9.81)
```

**双线程预积分器**（`imuPreintegration.cpp:177-178`）：
- `imuIntegratorOpt_`：用于因子图构造（帧间 IMU 积分）
- `imuIntegratorImu_`：每条 IMU 消息积分，>200Hz 实时发布位姿

**状态向量**：15 DoF = Pose(6) + Vel(3) + Bias(6)

**IMU 因子**（`imuPreintegration.cpp:350-380`）：
```
ImuFactor(X(k-1), V(k-1), X(k), V(k), B(k-1), preint_imu)
BetweenFactor<ConstantBias>(B(k-1), B(k), 0, noiseModelBetweenBias)
```

**故障检测**（`imuPreintegration.cpp:438-456`）：速度 > 30 m/s 或 bias > 1.0 时重置系统

### 11.4 跨传感器协同

**时间同步**：IMU 和 LiDAR 时间戳均来自 `sensor_msgs` header.stamp，系统使用 ROS 时间。`imageProjection` 在 IMU deskew 时自动查找时间窗口内 IMU 数据（清除 `timeScanCur - 0.01` 之前的过期数据）。IMU 预积分器中 `imuIntegratorOpt_` 在两帧 LiDAR 之间积分所有 IMU 数据。

**数据缓冲**：`cloudQueue` 缓冲 2 帧后才开始处理（`imageProjection.cpp:201-202`），确保有足够历史数据用于 deskew 和初始猜测。

**初始化（IMU 预积分）**（`imuPreintegration.cpp:252-316`）：首次 LiDAR 里程计到达后，添加 `PriorFactor<Pose3>` + `PriorFactor<Vector3>`（零速度先验）+ `PriorFactor<ConstantBias>`（零偏置先验），ISAM2 初始化后重置预积分器。

**初始化（建图）**（`mapOptmization.cpp:793-804`）：第一帧用 IMU roll/pitch 初始化姿态，yaw 根据 `useImuHeadingInitialization` 决定是否归零。

**降级**：
- 无 IMU 时：deskew 禁用（`time` 字段缺失），IMU 预积分不可用，回退纯 LiDAR 模式（yaw 漂移不可避免）
- 无 GPS 时：GPS 因子不添加，不影响正常功能
- 回环关闭时：全局因子图不添加 loop factor，位姿图仍通过里程计 BetweenFactor 维持
