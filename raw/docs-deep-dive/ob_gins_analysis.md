...
| 传感器 | 数据格式 | 输入源 | 频率 | 核心参数 |
|--------|---------|--------|------|---------|
| **IMU** | 文本文件 (7/8/9列) | 配置文件 `imufile` | 100-200 Hz (config: `imudatarate`) | dtheta(3), dvel(3), odovel(可选) |
| **GNSS-PPP/RTK** | 文本文件 (7/13列) | 配置文件 `gnssfile` | 1 Hz | BLH(3), std(3), vel_NED(可选,13列) |

### 7.2 IMU 数据管线

```
文本文件 (time dtheta_x dtheta_y dtheta_z dvel_x dvel_y dvel_z [odo])
  ↓ (imufileloader.h:41-65, ImuFileLoader::next())
列0: time (GPS周秒); 列1-3: dtheta (角增量, rad, FRD); 列4-6: dvel (速度增量, m/s)
  ↓ dt = next_time - curr_time; 若间隔过大 → dt = 1/imudatarate
  ↓ 里程计 (可选):
    8列: odovel = data[7] × dt              (单轮速)
    9列: odovel = ½(data[7]+data[8]) × dt    (双轮速均值)
  ↓ (ob_gins.cc:496-521) IMU 缓冲: std::deque<IMU> imu_buffer_pre_
[整秒节点构建] (ob_gins.cc:524-621):
  ↓ 固定积分长度: INTEGRATION_LENGTH = 1.0s
  ↓ IMU 插值拆分到整秒节点:
    isNeedInterpolation(): 前IMU离整秒≤0.0001s → 丢弃; 后IMU离整秒≤0.0001s → 直接使用
    否则: scale = (imu01.time - mid) / imu01.dt
          imu00.dtheta = imu01.dtheta × (1-scale); imu11.dtheta = imu01.dtheta × scale
  ↓ 预积分 (同 IC-GVINS 体系):
    工厂方法: Preintegration::createPreintegration() (preintegration.h)
    预积分残差 (15维): e_p(3)+e_v(3)+e_q(3)+e_bg(3)+e_ba(3)
    地球自转补偿 (isearth=true): 补偿 iewn (earth.h:229)
    里程计变体: PreintegrationOdo / PreintegrationEarthOdo
  ↓ 零偏校正: 一阶雅可比, 变化 > 6×std → 触发重积分 (reintegration)
  ↓ 零偏硬约束 (preintegration_normal.cc:264-272):
    IMU_GRY_BIAS_STD = 7200°/hr = 2.0 rad/s
    IMU_ACC_BIAS_STD = 20000 mGal = 0.2 m/s²
    IMU_ACC_Z_SCALE = 100 (OB独有, z轴比例因子额外约束)
  ↓ PreintegrationFactor: [pose₀(7), mix₀(9/10), pose₁(7), mix₁(9/10)]
```

**IMU 行/列约定**:
- 最少 7 列: 时间 + 3角增量 + 3速增量 (无里程计)
- 8 列: + 单轮速 → odovel = speed × dt
- 9 列: + 双轮速 → odovel = mean(speed_left, speed_right) × dt

### 7.3 GNSS 数据管线

```
文本文件 (time lat lon height [vel_N vel_E vel_D] std_N std_E std_D)
  ↓ (gnssfileloader.h:30-59, GnssFileLoader::next())
7列格式: time, lat(deg), lon(deg), h(m), σ_N, σ_E, σ_D
13列格式: time, lat, lon, h, v_N, v_E, v_D, σ_N, σ_E, σ_D
  ↓ 坐标转换: lat × D2R (deg→rad), lon × D2R (deg→rad), h 不变
  ↓ (ob_gins.cc:173-178) 原点设置:
    station_origin = 首个 GNSS 定位 (WGS84 BLH)
    gnss.blh = Earth::global2local(station_origin, gnss.blh) → 局部 NED
  ↓ 精确对齐检查: |gnss.time - round(gnss.time)| ≤ 0.001s (容差 1ms)
  ↓ (ob_gins.cc:188-214) 初始化:
    state.p = gnss.blh - R(euler(initatt)) × antlever
    state.q = euler2quaternion(initatt)
    state.v = initvel, state.bg = initbg, state.ba = initba
  ↓ 窗口管理: gnsslist_ 存储局部 NED 位置 + 标准差
[因子构建] addGnssFactors() (同 IC-GVINS):
  ↓ GnssFactor (SizedCostFunction<3,7>):
    e = p_IMU + R(q)·l_ant - p_GNSS  (3维, NED)
    W = diag(1/σ_N, 1/σ_E, 1/σ_D)
  ↓ 粗差剔除 (ob_gins.cc:295-417):
    Chi²(3自由度, 阈值 7.815) → reweight std → 移除核函数 → 重加因子
```

**GNSS 坐标系转换链**:
```
WGS84 BLH → blh2ecef() → ECEF(x,y,z) → global2local(origin) → NED(n,e,d)
  blh2ecef: rn = a/√(1-e²sin²φ)
            x = (rn+h)cosφcosλ, y = (rn+h)cosφsinλ, z = ((1-e²)rn+h)sinφ
  WGS84 参数: a=6378137, f=1/298.257223563, e²=0.00669437999014
```

### 7.4 GNSS 误差模型 (PPP 后处理管线)

OB_GINS 输入数据通常由 **PPP (Precise Point Positioning)** 或 **RTK** 后处理生成:
```
GNSS 原始观测 (RINEX)
  ↓ PPP 处理 (外部, 非 OB_GINS 代码):
    伪距观测: P = ρ + c·(dt_r - dtˢ) + I + T + ε_P
    载波相位: Φ = ρ + c·(dt_r - dtˢ) - I + T + λ·N + ε_Φ
  ↓ 精密星历/钟差产品 (CODE/IGS RTS)
  ↓ 对流层延迟: Saastamoinen 模型 + 天顶湿延迟估计
  ↓ 电离层延迟: 双频消电离层组合 (IF LC): P_IF = (f₁²P₁ - f₂²P₂)/(f₁²-f₂²)
  ↓ 模糊度: 浮点 → 固定 (LAMBDA 方法)
  ↓ → PPP 定位解 (cm级精度) + 标准差 → 文本文件 → OB_GINS 输入
```
...
| 机制 | 实现 | 参数 |
|------|------|------|
| **时间同步** | 固定整秒节点 + IMU 插值 | `INTEGRATION_LENGTH=1.0s, MINIMUM_INTERVAL=0.001s` |
| **插值策略** | 按时间比拆分 IMU 增量 | `scale = (t1 - t_mid) / dt` |
| **初始化** | 外部指定 (initvel/initatt/initgb/initab) | 无自动重力调平 |
| **滑窗大小** | config: `windows` (默认 30 秒) | 窗口满时边缘化最老节点 |
| **GNSS 阈值** | 标准差过大/中断时丢弃 | `gnssthreshold, gnssoutagetime` |
| **在线估计** | IMU 零偏动态估计 | 滑窗内联合优化 |
| **降级策略** | GNSS 中断 → 纯惯性 (INS 机械编排漂移) | 无视觉辅助 |

### 7.6 因子图拓扑

```
时间节点 (整秒采样, 固定 1s):
  TimeNode_k = [pose(7), mix(9/10/12)]

因子 (Ceres ResidualBlock):
  ├── PreintegrationFactor(k,k+1) ── [pose_k, mix_k, pose_k+1, mix_k+1]
  ├── GnssFactor(t) ──────────────── [pose_k]          (GNSS 对齐时)
  ├── ImuErrorFactor ─────────────── [mix_K]          (零偏约束)
  └── MarginalizationFactor ──────── [pose₁...mix_N]  (边缘化先验)

优化: Ceres LM + SPARSE_NORMAL_CHOLESKY, 两轮迭代
```
